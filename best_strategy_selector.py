...
def build_master_db():
    ...
    full_df.loc[full_df["source_round"].str.contains("round6"), "priority"] = (
        1  # Seasonality is good
    )

    # Sort: Priority (desc), Sharpe (desc)
    full_df = full_df.sort_values(
        ["ticker", "priority", "sharpe"], ascending=[True, False, False]
    )
...
def get_recommendation(ticker):
    ...
    if ticker in db:
        ...
        print(
            f"{'Rank':<5} {'Strategy':<30} {'Sharpe':>8} {'CAGR':>8} {'MDD':>8} {'Source'}"
        )
        print("-" * 80)

        for i, strat in enumerate(db[ticker]):
            print(
                f"{i+1:<5} {strat['strategy']:<30} {strat['sharpe']:>8.2f} {strat['cagr']:>7.1f}% {strat['mdd']:>7.1f}% {strat['source']}"
            )
        ...
        print(
            "Recommendation: Use 'AdaptiveEnsemble' or 'Month_11' as robust defaults."
        )
