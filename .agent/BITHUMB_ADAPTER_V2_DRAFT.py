...
class BithumbExecutionAdapter:
    ...
    def place_order(
        ...
        try:
            ...
            order_result = self._parse_result(
                result, symbol, side, coin_quantity, amount_krw, order_type, current_price
            )
            ...
            return self._rejected_order(symbol, side, amount_krw, f"Invalid Parameter: {e}")
        except Exception as e:
            logger.error(f"[BithumbExecution] Unexpected error: {e}", exc_info=True)
            return self._rejected_order(symbol, side, amount_krw, f"Unexpected Error: {e}")
    ...
    def _parse_result(
        ...
        """API 응답 파싱"""
        if result is None:
            return self._rejected_order(symbol, side, krw_amount, "Order failed - null response")
    ...
    def _rejected_order(self, symbol, side, krw_amount, reason) -> BithumbOrderResult:
        """거부된 주문"""
        logger.warning(f"[BithumbExecution] REJECTED: {symbol} {side} {krw_amount:,.0f} KRW - {reason}")
        return BithumbOrderResult(
            exchange="bithumb",
            order_id="",
            symbol=symbol,
