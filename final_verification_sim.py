...
OHLCV_DIRS = [
    DATA_DIR / "kr_stock/kospi_ohlcv",
    DATA_DIR / "kr_stock/kosdaq_ohlcv"
]
...
def load_viable_strategies():
    ...
    for r_name, r_path in DATA_DIRS.items():
        ...
        for f in files:
            try:
                ...
                # Normalize columns
                if 'pattern' in df.columns: df['strategy'] = df['pattern']
                if 'k' in df.columns: df['strategy'] = 'VolBreakout_k' + df['k'].astype(str)
                ...
                if 'win_rate' in df.columns: cols.append('win_rate')
    ...
    if not strategies: return pd.DataFrame()
...
def run_simulation():
    ...
    for date in tqdm(dates):
        ...
        for _, strat in strat_df.iterrows():
            ...
            if date not in price_data[ticker].index: continue
            ...
            if row['Amount'] < MIN_AMOUNT: continue
        ...
        capital += daily_pnl
        equity_curve.append({'date': date, 'equity': capital, 'positions': len(selected)})
