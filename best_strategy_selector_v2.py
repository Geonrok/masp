...
def get_recommendation(ticker):
    ...
    if ticker in db:
        ...
        print(
            f"{'Rank':<5} {'Strategy':<30} {'Sharpe':>8} {'Return':>8} {'MDD':>8} {'Source'}"
        )
        print("-" * 90)

        for i, strat in enumerate(db[ticker]):
            print(
                f"{i+1:<5} {strat['strategy']:<30} {strat['sharpe']:>8.2f} {strat['cagr_or_ret']:>7.1f}% {strat['mdd']:>7.1f}% {strat['source']}"
            )
        ...
        if "VolBreakout" in best["strategy"]:
            print(
                "Note: This is an intraday strategy. Buy Stop Order = Open + (PrevRange * k)"
            )
    else:
