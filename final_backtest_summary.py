...
def generate_summary():
    ...
    if 'portfolio' in results:
        ...
        for _, row in pf.nlargest(5, 'total_return_pct').iterrows():
            print(f"    {row['strategy']}: {row['total_return_pct']:.1f}% (CAGR {row['cagr_pct']:.1f}%, Sharpe {row['sharpe_ratio']:.2f})")
    ...
    if 'additional' in results:
        ...
        for _, row in add.nlargest(5, 'total_return_pct').iterrows():
            print(f"    {row['ticker']} + {row['strategy']}: {row['total_return_pct']:.1f}%")
    ...
    if 'portfolio' in results:
        ...
        pf['ticker'] = 'PORTFOLIO'
        pf['name'] = pf['strategy']
        all_results.append(pf[['category', 'ticker', 'name', 'strategy', 'total_return_pct']])

    if 'additional' in results:
        add = results['additional'].copy()
        add['category'] = '투자자별'
        add['name'] = add['ticker']
        all_results.append(add[['category', 'ticker', 'name', 'strategy', 'total_return_pct']])
    ...
    if 'ensemble' in results:
        ...
        if 'name' not in ens.columns:
            ens['name'] = ens['ticker']
        all_results.append(ens[['category', 'ticker', 'name', 'strategy', 'total_return_pct']])
    ...
    if all_results:
        ...
        for i, (_, row) in enumerate(combined.head(30).iterrows(), 1):
            print(f"{i:>2}. [{row['category']:<6}] {row['name']:<12} + {row['strategy']:<30}: {row['total_return_pct']:>8.1f}%")

        # 수익률 분포
        print("\n[수익률 분포]")
        print("-" * 50)
        bins = [(-float('inf'), 0), (0, 100), (100, 500), (500, 1000), (1000, float('inf'))]
        labels = ['손실', '0-100%', '100-500%', '500-1000%', '1000%+']

        for (low, high), label in zip(bins, labels):
            count = len(combined[(combined['total_return_pct'] > low) & (combined['total_return_pct'] <= high)])
