...
def analyze_foreign_data():
    ...
    for file in sorted(foreign_files):
        ...
        print(f"\n{name} ({ticker})")
        print(f"  데이터: {stats['start_date']} ~ {stats['end_date']} ({stats['data_count']}일)")
    ...
    summary_df.to_csv(f"{DATA_PATH}/foreign_summary.csv", index=False, encoding='utf-8-sig')
...
def merge_data_for_strategy():
    ...
    foreign_files = {f.replace('_foreign.csv', '') for f in os.listdir(DATA_PATH) if f.endswith('_foreign.csv')}
    daily_files = {f.replace('_daily.csv', '') for f in os.listdir(DATA_PATH) if f.endswith('_daily.csv')}
    ...
    for ticker in common_tickers:
        # 외국인 데이터
        foreign_df = pd.read_csv(f"{DATA_PATH}/{ticker}_foreign.csv", encoding='utf-8-sig')
        ...
        # 병합 (날짜 기준)
        merged = pd.merge(daily_df, foreign_df[['dt', 'chg_qty', 'wght', 'poss_stkcnt']], on='dt', how='left')
        merged['ticker'] = ticker

        # 가격 데이터 정리
        for col in ['cur_prc', 'open_pric', 'high_pric', 'low_pric']:
            if col in merged.columns:
                merged[col] = merged[col].astype(str).str.replace('+', '').str.replace('-', '').astype(float)
        ...
        # 저장
        merged.to_csv(f"{DATA_PATH}/{ticker}_merged.csv", index=False, encoding='utf-8-sig')
...
def main():
    ...
    if foreign_summary is not None and len(foreign_summary) > 0:
        ...
        for _, row in top5.iterrows():
            print(f"  {row['name']} ({row['ticker']}): {row['total_net_buy_30d']:,.0f}주")
