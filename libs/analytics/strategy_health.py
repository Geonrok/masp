...
class StrategyHealthMonitor:
    ...
    def check_health(self) -> HealthCheckResult:
        ...
        if consecutive_losses >= self.CONSECUTIVE_LOSS_CRITICAL:
            ...
            if status != HealthStatus.CRITICAL:
                status = HealthStatus.CRITICAL
            logger.error(f"[HealthMonitor] Consecutive losses critical: {consecutive_losses}")
        elif consecutive_losses >= self.CONSECUTIVE_LOSS_WARNING:
            triggers.append(f"CONSECUTIVE_LOSS_WARNING_{consecutive_losses}")
            if status == HealthStatus.HEALTHY:
                status = HealthStatus.WARNING
            logger.warning(f"[HealthMonitor] Consecutive losses warning: {consecutive_losses}")
    ...
    def _generate_recommendation(self, status: HealthStatus, triggers: List[str]) -> str:
        """
        상태별 권장 사항 생성
        ...
        if status == HealthStatus.CRITICAL:
            ...
            for trigger in triggers:
                if "DAILY_LOSS" in trigger:
                    recommendations.append("  - 일일 손실 한도 초과: 오늘 거래 중단 권장")
                elif "MDD_CRITICAL" in trigger:
                    recommendations.append("  - MDD 15% 초과: 포지션 청산 및 전략 재검토")
                elif "CONSECUTIVE_LOSS" in trigger:
                    recommendations.append("  - 연속 손실 8회: 전략 파라미터 재조정 필요")
                elif "SHARPE_CRITICAL" in trigger:
                    recommendations.append("  - Sharpe < 0: 전략 효과성 상실, 중단 권장")
        ...
        if status == HealthStatus.WARNING:
            ...
            for trigger in triggers:
                if "MDD_WARNING" in trigger:
                    ...
                    recommendations.append("  - Sharpe < 0.5: 전략 효과성 낮음, 조정 검토")
