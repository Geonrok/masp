...
class Scheduler:
    ...
    def _acquire_lock(self) -> bool:
        ...
        try:
            ...
            print(f"[Scheduler] Another instance is running (lock file: {self._lock_path})")
            return False
    ...
    def run_daemon(self) -> None:
        ...
        try:
            ...
            print(f"[Scheduler] Starting daemon mode for {self.config.asset_class.value}")
            print(f"[Scheduler] Schedule: {self.config.schedule.mode}")
            
            if self.config.schedule.mode == "cron":
                print(f"[Scheduler] Cron expression: {self.config.schedule.cron_expression}")
            else:
                print(f"[Scheduler] Interval: {self.config.schedule.interval_seconds} seconds")
