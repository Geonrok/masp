...
class ScheduleConfig(BaseModel):
    ...
    interval_seconds: int = Field(
        default=60, description="Interval in seconds (for interval mode)"
    )
    cron_expression: Optional[str] = Field(
        default=None, description="Cron expression (for cron mode)"
    )
    timezone: str = Field(
        default="Asia/Seoul", description="Timezone for scheduling (KST)"
    )
...
class Config(BaseModel):
    ...
    config_version: str = Field(
        default="0.1.0", description="Configuration schema version"
    )
    ...
    storage_dsn: str = Field(
        default="storage/local.db", description="SQLite database path"
    )
    run_lock: bool = Field(
        default=True, description="Enable run lock to prevent concurrent execution"
    )
    paper_mode: bool = Field(
        default=True, description="Enable paper trading (mock orders)"
    )
    ...
    upbit_access_key: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("UPBIT_ACCESS_KEY", ""))
    ...
    upbit_secret_key: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("UPBIT_SECRET_KEY", ""))
    ...
    binance_api_key: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("BINANCE_API_KEY", ""))
    ...
    binance_api_secret: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("BINANCE_API_SECRET", ""))
    ...
    bithumb_api_key: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("BITHUMB_API_KEY", ""))
            if os.getenv("BITHUMB_API_KEY")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    bithumb_secret_key: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("BITHUMB_SECRET_KEY", ""))
            if os.getenv("BITHUMB_SECRET_KEY")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    korea_broker_app_key: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("KOREA_BROKER_APP_KEY", ""))
            if os.getenv("KOREA_BROKER_APP_KEY")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    korea_broker_app_secret: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("KOREA_BROKER_APP_SECRET", ""))
            if os.getenv("KOREA_BROKER_APP_SECRET")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    korea_broker_account_number: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("KOREA_BROKER_ACCOUNT_NUMBER", ""))
            if os.getenv("KOREA_BROKER_ACCOUNT_NUMBER")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    ebest_app_key: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("EBEST_APP_KEY", ""))
            if os.getenv("EBEST_APP_KEY")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    ebest_app_secret: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("EBEST_APP_SECRET", ""))
            if os.getenv("EBEST_APP_SECRET")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    ebest_account_no: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("EBEST_ACCOUNT_NO", ""))
            if os.getenv("EBEST_ACCOUNT_NO")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    ebest_account_pwd: Optional[SecretStr] = Field(
        default_factory=lambda: (
            SecretStr(os.getenv("EBEST_ACCOUNT_PWD", ""))
            if os.getenv("EBEST_ACCOUNT_PWD")
            else None
        ),
        repr=False,
        exclude=True,
    ...
    service_name: Optional[str] = Field(
        default=None, description="Service name override"
    )
    build_version: str = Field(default="0.1.0", description="Build version")

    # Symbols to trade (mock for Phase 0)
    symbols: list[str] = Field(
        default_factory=list, description="List of symbols to process"
    )
...
def get_default_symbols(asset_class: AssetClass) -> list[str]:
    ...
    defaults = {
        ...
        AssetClass.KR_STOCK_SPOT: [
            "005930",
            "000660",
            "035420",
            "051910",
            "006400",
        ],  # Samsung, SK Hynix, NAVER, LG Chem, Samsung SDI
        AssetClass.KR_STOCK_FUTURES: [
            "101S6000",
            "101S3000",
            "101S9000",
        ],  # KOSPI200 futures codes
    }
