...
class UpbitExecutionAdapter:
    ...
    def place_order(
        ...
        max_order = (
            int(self.config.max_order_value_krw)
            if hasattr(self.config, "max_order_value_krw")
            else 1_000_000
        )
        ...
        try:
            ...
            order_result = self._parse_result(
                result, symbol, side, quantity, order_type, current_price
            )
    ...
    def _rejected_order(
        ...
        """거부된 주문 생성"""
        logger.warning(
            f"[UpbitExecution] REJECTED: {symbol} {side} {quantity} - {reason}"
        )
        return UpbitOrderResult(
            order_id="",
            symbol=symbol,
