...
class PaperExecutionAdapter(ExecutionAdapter):
    ...
    def __init__(self, market_data_adapter: MarketDataAdapter, initial_balance: float = 1_000_000, 
                 config=None, trade_logger=None):
        """
    ...
    def place_order(
        ...
        *,
        ...
        if quantity is not None and units is None:
            units = quantity
            logger.warning("[PaperExecution] DEPRECATED: 'quantity' parameter used. Use 'units=' instead.")
        ...
        if side_u == "BUY" and amount_krw is not None and units is None:
            ...
            if quote and quote.last > 0:
                ...
                logger.info(f"[PaperExecution] BUY: {amount_krw:,.0f} KRW → {units:.8f} units (fee buffer: {fee_buffer})")
            else:
                # 가격 조회 실패 시 거부
    ...
    def _fill_market_order(self, order: PaperOrder):
        ...
        if order.side == "BUY":
            ...
            if cost > self.balance:
                order.status = "REJECTED"
                order.rejection_reason = f"Insufficient balance: need {cost:,.0f}, have {self.balance:,.0f}"
                logger.warning(f"[PaperExecution] Order {order.order_id} rejected: insufficient balance")
                return
    ...
    def _update_position(self, symbol: str, qty_delta: float, price: float):
        ...
        if symbol not in self.positions:
            ...
            logger.info(f"[PaperExecution] New position: {qty_delta} {symbol} @ {price:,.0f}")
        else:
            ...
            if (old_qty > 0 and qty_delta > 0) or (old_qty < 0 and qty_delta < 0):
                ...
                if abs(pos.quantity) < 1e-8:
                    ...
                    logger.info(f"[PaperExecution] Position closed: {symbol}, total PnL: {pos.realized_pnl:,.0f}")
