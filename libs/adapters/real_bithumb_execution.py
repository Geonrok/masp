...
class BithumbExecutionAdapter:
        ...
        *,  # keyword-only 강제 (ChatGPT 권장)
        ...
        if self.config.is_kill_switch_active():
            ...
            return self._rejected_order(
                symbol, side_u, units or 0, "Kill-Switch active"
            )
        ...
        if amount_krw is not None and units is None:
            ...
            logger.info(
                f"[BithumbExecution] BUY: {amount_krw:,.0f} KRW → {units:.8f} units (fee buffer: {fee_buffer})"
            )

        # 6. KRW 금액 계산
        krw_amount = (
            float(amount_krw) if amount_krw is not None else units * current_price
        )

        # 7. side별 로깅 (디버깅용)
        if side_u == "BUY":
            logger.info(
                f"[BithumbExecution] BUY: {units:.8f} coins = {krw_amount:,.0f} KRW"
            )
        else:
        ...
        try:
            ...
            order_result = self._parse_result(
                result, symbol, side, units, order_type_u, current_price
            )
    ...
    def _parse_order_status(self, data: Dict) -> Optional[OrderStatus]:
        ...
        try:
            ...
            if data.get("trades"):
                trades = data["trades"]
                total_value = sum(
                    float(t.get("price", 0)) * float(t.get("volume", 0)) for t in trades
                )
    ...
    def _retry_operation(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.error(
                    f"[BithumbExecution] {operation_name} non-retryable error: {e}"
                )
                raise
    ...
    def _calculate_backoff(
        attempt: int, base_delay: float, max_delay: float = 30.0
    ) -> float:
        """지수 백오프 + 지터 계산."""
    ...
    def safe_cancel_order(
        ...
        if status.is_done:
            logger.info(
                f"[BithumbExecution] Order {order_id} already filled, cannot cancel"
            )
            return False
    ...
    def verify_order_execution(
        ...
        if status is None:
            logger.error(
                f"[BithumbExecution] Order {order_id} verification failed: not found"
            )
            return None
        ...
        if status.is_canceled:
            logger.warning(
                f"[BithumbExecution] Order {order_id} was canceled during verification"
            )
            return status
    ...
    def _parse_result(
        self, result, symbol, side, quantity, order_type, price
    ) -> BithumbOrderResult:
        """
        API 응답 파싱
        ...
        if result is None:
            return self._rejected_order(
                symbol, side, quantity, "Order failed: None response"
            )
        ...
        if isinstance(result, dict):
            order_id = (
                result.get("uuid") or result.get("order_id") or result.get("orderId")
            )
            if not order_id and "error" in result:
                ...
                if isinstance(error, dict):
                    ...
                    error_msg = str(error)
                return self._rejected_order(
                    symbol, side, quantity, f"API error: {error_msg}"
                )
        ...
        if not order_id or order_id == symbol or order_id == "None":
            logger.warning(
                f"[BithumbExecution] Invalid order_id: {order_id}, raw: {raw_response}"
            )
    ...
    def _rejected_order(self, symbol, side, quantity, reason) -> BithumbOrderResult:
        """거부된 주문"""
        logger.warning(
            f"[BithumbExecution] REJECTED: {symbol} {side} {quantity} - {reason}"
        )
        return BithumbOrderResult(
            order_id="",
            symbol=symbol,
