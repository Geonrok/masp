...
class MACrossoverStrategy(BaseStrategy):
    ...
    def _simulate_ma_values(self, symbol: str, date_str: str) -> tuple[float, float, float, float]:
        """
    ...
    def _detect_crossover(
        ...
        if prev_short_ma <= prev_long_ma and short_ma > long_ma:
            return Action.BUY, f"Golden Cross: MA{self.short_period} crossed above MA{self.long_period}"
        
        # Check for death cross (bearish signal)
        if prev_short_ma >= prev_long_ma and short_ma < long_ma:
            return Action.SELL, f"Death Cross: MA{self.short_period} crossed below MA{self.long_period}"
        
        # No crossover - hold current position
        if short_ma > long_ma:
            return Action.HOLD, f"Uptrend: MA{self.short_period} above MA{self.long_period}, holding"
        else:
            return Action.HOLD, f"Downtrend: MA{self.short_period} below MA{self.long_period}, holding"
    ...
    def _generate_metrics(
        ...
        return {
            ...
            "price_vs_short_ma_pct": round((current_price - short_ma) / short_ma * 100, 2),
        }
