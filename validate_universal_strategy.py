...
def load_all_kosdaq():
    ...
    for f in files:
        ...
        try:
            ...
            df = df.rename(columns={date_col: 'date', 'Close': 'close', 'Open': 'open',
...
def strategy_volume_breakout(df, vol_mult=2, price_period=20):
    ...
    signals = pd.Series(0, index=df.index)
    signals[(df['volume'] > df['vol_ma'] * vol_mult) & (df['close'] > df['price_ma'])] = 1
    return signals
...
def run_universal_validation():
    ...
    sorted_strategies = sorted(strategy_results.items(),
                               key=lambda x: x[1]['profitable_pct'], reverse=True)
    ...
    practical = [(name, stats) for name, stats in sorted_strategies
    ...
    if practical:
        ...
        practical_40 = [(name, stats) for name, stats in sorted_strategies
                        if stats['profitable_pct'] >= 40]

        for strategy_name, stats in practical_40[:5]:
            print(f"  {strategy_name}: 수익종목 {stats['profitable_pct']:.1f}%, "
    ...
    for strategy_name, strategy_func in strategies[:10]:  # 상위 10개만
        ...
        if is_returns and oos_returns:
            ...
            oos_profitable = sum(1 for r in oos_returns if r > 0) / len(oos_returns) * 100

            print(f"{strategy_name:<20} | IS 수익종목: {is_profitable:>5.1f}% | "
    ...
    results_df.to_csv('E:/투자/Multi-Asset Strategy Platform/universal_strategy_results.csv',
