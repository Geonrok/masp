...
def main() -> bool:
    ...
    print(f"  Filled: {buy_result.filled_quantity:.8f} BTC @ {buy_result.filled_price:,.0f}")
    ...
    if sell_quantity <= 0:
        ...
        print(f"  Filled: {sell_result.filled_quantity:.8f} BTC @ {sell_result.filled_price:,.0f}")
    ...
    reporter = DailyReportGenerator(
        logger, 
        health, 
        report_dir=str(live_log_dir / "reports")
    )
