...
class TestKillSwitch:
    ...
    def test_kill_switch_detection_active(self, temp_kill_switch):
        ...
        config = Config(
            asset_class=AssetClass.CRYPTO_SPOT, kill_switch_file=temp_kill_switch
        )

        assert (
            config.is_kill_switch_active() is True
        ), "Kill-switch should be detected as active"
    ...
    def test_kill_switch_detection_inactive(self):
        ...
        assert (
            config.is_kill_switch_active() is False
        ), "Non-existent file should be inactive"

    def test_kill_switch_no_file_configured(self):
        """Test kill-switch when no file is configured"""
        config = Config(asset_class=AssetClass.CRYPTO_SPOT, kill_switch_file=None)

        assert (
            config.is_kill_switch_active() is False
        ), "No file configured should be inactive"
    ...
    def test_order_validator_blocks_when_active(self, temp_kill_switch):
        ...
        assert (
            "Kill-Switch" in result.reason
        ), f"Rejection reason should mention kill-switch, got: {result.reason}"
    ...
    def test_order_validator_allows_when_inactive(self):
        ...
        ), f"Order should be allowed, but got: {result.reason}"

    def test_order_validator_max_position_limit(self):
        """Test position size limit (10% of equity)"""
        config = Config(asset_class=AssetClass.CRYPTO_SPOT)
...
if __name__ == "__main__":
    ...
    config3 = Config(asset_class=AssetClass.CRYPTO_SPOT, kill_switch_file=temp_path)
