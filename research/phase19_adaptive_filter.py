...
def super_smoother(series, period=10):
    ...
    result[1] = close[1]
    for i in range(2, n):
        result[i] = c1 * (close[i] + close[i - 1]) / 2 + c2 * result[i - 1] + c3 * result[i - 2]
...
def strat_kama_ema_hybrid(df, lookback=48):
    ...
    return np.where((close > upper) & (close > vwap * 1.02) & (ema_f > ema_s) & (k_slope > 0), 1, 0)
...
def run_portfolio_oos(all_data, strat_fn, btc_df=None,
                      min_oos_bars=16000, max_positions=10,
                      test_bars=720, position_scale=5.0, train_bars=4320):
    oos_data = {}
...
def main():
    ...
    header = (f"{'Strategy':<22} {'Rev':>4} {'Sharpe':>7} {'Return':>8} {'MaxDD':>7} "
    ...
    for name, fn in strategies.items():
        r = run_portfolio_oos(all_data, fn, btc_df=btc_df,
                              min_oos_bars=16000, position_scale=5.0)
        if r:
            c, p = check_criteria_revised(r)
            rr = r['regime_returns']
            bull = f"{rr.get('BULL', 0)*100:+.1f}%" if rr.get('BULL') is not None else "N/A"
            bear = f"{rr.get('BEAR', 0)*100:+.1f}%" if rr.get('BEAR') is not None else "N/A"
            side = f"{rr.get('SIDEWAYS', 0)*100:+.1f}%" if rr.get('SIDEWAYS') is not None else "N/A"

            fails = [k for k, v in c.items() if not v]
            results.append((name, p, r))

            print(f"  {name:<20} [{p}/6] {r['sharpe']:>6.2f} {r['total_return']*100:>+7.1f}% "
    ...
    if passed:
        ...
        for name, p, r in passed:
            ...
            bear_ok = " [BEAR OK]" if bear_ret is not None and bear_ret > 0 else " [BEAR LOSS]" if bear_ret is not None and bear_ret < 0 else ""
        ...
        bear_ranked = sorted(passed, key=lambda x: -(x[2]['regime_returns'].get('BEAR', -999) or -999))
        for name, p, r in bear_ranked[:3]:
            rr = r['regime_returns']
            print(f"  {name}: Bear={rr.get('BEAR',0)*100:+.1f}% "
    ...
    if baseline:
        ...
        better = [(n, r) for n, p, r in results if r['sharpe'] > baseline['sharpe'] and n != 'A_baseline_EMA']
        if better:
