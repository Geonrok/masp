...
def generate_multifactor_signals(df):
    ...
    return pd.Series(np.where(scores >= 3, 1, np.where(scores <= -3, -1, 0)), index=df.index)
...
def simulate_full(df, signals, position_pct=0.02, max_bars=48):
    ...
    for i in range(len(df)):
        ...
        if position != 0:
            bars_held += 1
            should_exit = (bars_held >= max_bars) or (sig != 0 and sig != position) or (sig == 0)
...
def task_4_3_walk_forward(df, signal_func, name, train_bars=1080, test_bars=180):
    ...
    # Sharpe
    sharpe = np.mean(results) / (np.std(results) + 1e-10) * np.sqrt(12) if len(results) > 1 else 0
...
def main():
    ...
    for strat_key, (strat_name, signal_func) in strategies.items():
        ...
        for symbol in symbols:
            ...
            print(f"    IS:  Ret={is_r['total_return']*100:+.1f}%  DD={is_r['max_drawdown']*100:.1f}%  WR={is_r['win_rate']*100:.0f}%  PF={is_r['profit_factor']:.2f}  T={is_r['trade_count']}")

            # 4.2 OOS
            oos_r = task_4_2_out_of_sample(df, signal_func, strat_name)
            strat_results['oos'][symbol] = oos_r
            print(f"    OOS: Ret={oos_r['total_return']*100:+.1f}%  DD={oos_r['max_drawdown']*100:.1f}%  WR={oos_r['win_rate']*100:.0f}%  PF={oos_r['profit_factor']:.2f}  T={oos_r['trade_count']}")
            ...
            print(f"    WFA: {wfa_r['wfa_efficiency']:.0f}% efficient ({wfa_r.get('profitable_periods',0)}/{wfa_r['periods']} profitable)  Sharpe={wfa_r.get('sharpe',0):.2f}")
            ...
            for period, res in stress_r['stress_results'].items():
                if 'skipped' not in res:
                    print(f"    Stress {period}: Ret={res['return']*100:+.1f}%  DD={res['max_dd']*100:.1f}%")
            ...
            if not stats_r.get('insufficient_data'):
                sig_marker = "***" if stats_r['significant_5pct'] else ("*" if stats_r['significant_10pct'] else "")
                print(f"    Stats: t={stats_r['t_stat']:.2f}  p={stats_r['p_value']:.3f}  CI=[{stats_r['bootstrap_ci_95'][0]*100:.2f}%, {stats_r['bootstrap_ci_95'][1]*100:.2f}%] {sig_marker}")
        ...
        wfa_effs = [r['wfa_efficiency'] for r in strat_results['wfa'].values() if r['periods'] > 0]
        wfa_sharpes = [r.get('sharpe', 0) for r in strat_results['wfa'].values() if r['periods'] > 0]
        p_values = [r.get('p_value', 1) for r in strat_results['stats'].values() if not r.get('insufficient_data')]
        ...
        print(f"  OOS Profitable: {oos_profitable}/{len(oos_returns)} ({oos_profitable/len(oos_returns)*100:.0f}%)")
        ...
        criteria = {
            ...
            'max_dd_lt_25': all(r['max_drawdown'] > -0.25 for r in strat_results['oos'].values()),
            'win_rate_gt_45': np.mean([r['win_rate'] for r in strat_results['oos'].values()]) > 0.45,
            'profit_factor_gt_1_5': np.mean([r['profit_factor'] for r in strat_results['oos'].values()]) > 1.5,
            'wfa_efficiency_gt_50': avg_wfa > 50,
            'trade_count_gt_100': all(r['trade_count'] > 100 for r in strat_results['oos'].values()),
        }
        ...
        all_results[strat_key] = {
            ...
            'criteria': criteria,
            'details': {
                'is': {s: {k: v for k, v in r.items() if k not in ('equity', 'drawdown', 'trades')}
                       for s, r in strat_results['is'].items()},
                'oos': {s: {k: v for k, v in r.items() if k not in ('equity', 'drawdown', 'trades')}
                       for s, r in strat_results['oos'].items()},
                'wfa': {s: {k: v for k, v in r.items() if k != 'period_returns'}
                       for s, r in strat_results['wfa'].items()},
                'stress': strat_results['stress'],
                'stats': {s: {k: (list(v) if isinstance(v, tuple) else v) for k, v in r.items()}
    ...
    best = None
    for key, res in sorted(all_results.items(), key=lambda x: x[1]['criteria_passed'], reverse=True):
        marker = "*** BEST ***" if best is None else ""
        if best is None:
            best = key
        print(f"  {res['name']}: {res['criteria_passed']}/{res['criteria_total']} criteria passed  "
