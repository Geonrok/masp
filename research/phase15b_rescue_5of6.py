...
def main():
    ...
    print(f"{'Config':<45} {'Pass':>4} {'Sharpe':>7} {'Return':>8} {'MaxDD':>7} {'WR':>5} {'PF':>6} {'WFA':>5} {'T':>5}")
    print("-" * 95)
    ...
    for strat_name, strat_fn in strategies.items():
        for cfg in configs:
            ...
            r = run_portfolio_oos(all_data, strat_fn,
            ...
            if r:
                ...
                print(f"  {label:<43} [{p}/6] {r['sharpe']:>6.2f} {r['total_return']*100:>+7.1f}% "
    ...
    if six_six:
        for name, p, r in six_six:
            print(f"  [{p}/6] {name}: Sharpe={r['sharpe']:.2f} Ret={r['total_return']*100:+.1f}%")

        # Does any beat vol_profile baseline?
        vp_baseline = [r for n, p, r in six_six if 'vol_profile' in n and 'pos10_rb720' in n]
        vp_sharpe = vp_baseline[0]['sharpe'] if vp_baseline else 2.52
        better = [(n, r) for n, p, r in six_six if r['sharpe'] > vp_sharpe and 'vol_profile' not in n]
        if better:
