...
print("\n{:<20} {:>10} {:>10} {:>10}".format("Strategy/Market", "Upbit", "Bithumb", "Binance"))
print("-" * 60)
...
print(f"  Sharpes: Upbit={k10_sharpes[0]:.2f}, Bithumb={k10_sharpes[1]:.2f}, Binance={k10_sharpes[2]:.2f}")
...
print(f"  Sharpes: Upbit={k5_sharpes[0]:.2f}, Bithumb={k5_sharpes[1]:.2f}, Binance={k5_sharpes[2]:.2f}")
...
for _, row in holdout_df.iterrows():
    print(f"  {row['exchange']}: Sharpe={row['sharpe']:.3f}, MDD={row['mdd']*100:.1f}%, Return={row['total_return']*100:.0f}%")
...
for market, data in k5_holdout.items():
    print(f"  {market}: Sharpe={data['sharpe']:.3f}, MDD={data['mdd']:.1f}%, Return={data['return']:.0f}%")
...
k10_holdout_bithumb = holdout_df[holdout_df['exchange'] == 'bithumb']['sharpe'].values[0]
...
print("\nKAMA=10, TSMOM=60:")
print(f"  Upbit:   {k10_is_upbit:.2f} → {k10_holdout_upbit:.2f} (decay: {k10_decay_upbit:.1f}%)")
print(f"  Bithumb: {k10_is_bithumb:.2f} → {k10_holdout_bithumb:.2f} (decay: {k10_decay_bithumb:.1f}%)")
print(f"  평균 Decay: {(k10_decay_upbit + k10_decay_bithumb)/2:.1f}%")

print("\nKAMA=5, TSMOM=90:")
print(f"  Upbit:   {k5_is_upbit:.2f} → {k5_holdout_upbit:.2f} (decay: {k5_decay_upbit:.1f}%)")
print(f"  Bithumb: {k5_is_bithumb:.2f} → {k5_holdout_bithumb:.2f} (decay: {k5_decay_bithumb:.1f}%)")
print(f"  Binance: {k5_is_binance:.2f} → {k5_holdout_binance:.2f} (decay: {k5_decay_binance:.1f}%)")
...
def calculate_score(name, holdout_sharpes, decay_avg, cm_pass, dsr_pass, has_binance_holdout):
    score = 0
    details = []
...
# Binance 추정값 추가
k10_binance_est = k10[k10['market'] == 'binance_spot']['sharpe'].values[0] * (1 - (k10_decay_upbit + k10_decay_bithumb)/2/100)
...
k10_score, k10_details = calculate_score(
    "KAMA=10", k10_holdouts, k10_decay_avg,
    k10_cm['pass'], k10_p < 0.05, False
)
...
k5_score, k5_details = calculate_score(
    "KAMA=5", k5_holdouts, k5_decay_avg,
    k5_cm['pass'], k5_p < 0.05, True
)
...
""".format(
    k10_decay_avg, k5_decay_avg,
    k10_cm['cv'], '✓' if k10_cm['pass'] else '✗',
    k5_cm['cv'], '✓' if k5_cm['pass'] else '✗',
    k10_p, '✓' if k10_p < 0.05 else '✗',
    k5_p, '✓' if k5_p < 0.05 else '✗',
    k10_binance_est,
    get_grade(k10_score), k10_score,
    get_grade(k5_score), k5_score,
    k10_decay_avg, k5_decay_avg
))
