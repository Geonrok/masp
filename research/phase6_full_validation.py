...
def simulate(df, signals, position_pct=0.02, max_bars=72,
             atr_stop=3.0, profit_target_atr=8.0):
    capital = 1.0
    position = 0
    entry_price = 0
    bars_held = 0
...
def portfolio_walk_forward(symbols, train_bars=4320, test_bars=720,
    ...
    sortino_denom = np.std([r for r in period_returns if r < 0]) if any(r < 0 for r in period_returns) else 1e-10
...
def main():
    ...
    print(f"  Profitable: {len(profitable_symbols)} ({len(profitable_symbols)/len(per_symbol)*100:.0f}%)")
    print(f"  Win Rate > 45%: {len(high_wr_symbols)}")

    # Top performers
    ranked_symbols = sorted(per_symbol.items(), key=lambda x: x[1]['total_return'], reverse=True)

    print(f"\n  Top 15 symbols:")
    for s, r in ranked_symbols[:15]:
        print(f"    {s:<15} Ret={r['total_return']*100:+.1f}%  WR={r['win_rate']*100:.0f}%  "
    ...
    for period, res in stress.items():
        if 'skipped' not in res:
            print(f"  {period}: PnL={res['total_pnl']*100:+.2f}%  T={res['trade_count']}  WR={res['win_rate']*100:.0f}%")
        else:
    ...
    best_portfolio = max(portfolio_results.items(), key=lambda x: x[1]['criteria_passed'])
...
""")
    ...
    for name, r in sorted(portfolio_results.items(), key=lambda x: x[1]['criteria_passed'], reverse=True):
        print(f"  {name:<20} [{r['criteria_passed']}/6]  Sharpe={r['sharpe']:.2f}  "
    ...
    report = {
        ...
        'per_symbol_results': {s: {k: float(v) if isinstance(v, (np.floating, float)) else int(v) if isinstance(v, (np.integer, int)) else v
                                   for k, v in r.items()}
                              for s, r in per_symbol.items()},
        'portfolio_results': {name: {k: float(v) if isinstance(v, (np.floating, float)) else int(v) if isinstance(v, (np.integer, int)) else v
                                     for k, v in r.items() if k != 'criteria'}
