...
def run_long_only(data, start, end):
    ...
    for symbol, df in data.items():
        ...
        tsmom_sig = np.array([prices[i] > prices[i-90] if i >= 90 else False for i in range(n)])
        df = df.copy()
    ...
    for i, date in enumerate(dates):
        ...
        if prev_positions:
            pnl = sum(portfolio * w * direction * (prices_today[sym] - prev_p) / prev_p
                      for sym, (w, prev_p, direction) in prev_positions.items() if sym in prices_today)
            portfolio += pnl
        ...
        if selected:
            w = 1.0 / len(selected)
            prev_positions = {s: (w, prices_today[s], 1) for s in selected if s in prices_today}  # 1 = long
        else:
            prev_positions = {}
...
def run_long_short(data, start, end):
    ...
    for symbol, df in data.items():
        ...
        tsmom_long = np.array([prices[i] > prices[i-90] if i >= 90 else False for i in range(n)])
        tsmom_short = np.array([prices[i] < prices[i-90] if i >= 90 else False for i in range(n)])
    ...
    for i, date in enumerate(dates):
        ...
        if longs_today:
            selected = [s for s, v in sorted(longs_today, key=lambda x: x[1], reverse=True)[:20]]
            direction = 1
            long_days += 1
        elif shorts_today:
            selected = [s for s, v in sorted(shorts_today, key=lambda x: x[1], reverse=True)[:20]]
            direction = -1
            short_days += 1
        else:
            selected = []
        ...
        if selected:
            w = 1.0 / len(selected)
            prev_positions = {s: (w, prices_today[s], direction) for s in selected if s in prices_today}
        else:
            prev_positions = {}
    ...
    return {'return': total_ret, 'sharpe': sharpe, 'mdd': mdd, 'long_days': long_days, 'short_days': short_days}
...
print()
print("=" * 85)
print(f"{'Period':<15} {'Spot L-Only':>14} {'Futures L/S':>14} {'Diff':>10} {'L-days':>8} {'S-days':>8}")
print("-" * 85)
...
for name, start, end in periods:
    ...
    if r_spot and r_futures:
        ...
        print(f"{name:<15} {spot_ret:>+13.1f}% {futures_ret:>+13.1f}% {diff:>+9.1f}%p {l_days:>8} {s_days:>8}")
...
if r:
    ...
    if r_spot:
        ...
        print(f"  → Spot 대비 {improvement:+.1f}%p {'개선' if improvement > 0 else '악화'}")
