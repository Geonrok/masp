...
def run_correct_backtest(data, kama_p, tsmom_p):
    ...
    if len(btc_2025) < 2:
        return None
    btc_ret = (btc_2025["close"].iloc[-1] - btc_2025["close"].iloc[0]) / btc_2025[
        "close"
    ].iloc[0]
    ...
    return {
        ...
        "mdd": mdd,
        "btc_ret": btc_ret,
        "n_symbols": len(signal_data),
        "entry_count": entry_count,
        "exit_count": exit_count,
        "win_rate": win_rate,
        "trading_days": len(dates_2025),
    }
...
print()
print(
    "Method: Signal at Day N close -> Position for Day N+1 -> Return attributed correctly"
)
print()
...
for market_name, folder_name in MARKETS.items():
    ...
    for strat_name, kama, tsmom in [
        ("KAMA=5, TSMOM=90", 5, 90),
        ("KAMA=10, TSMOM=60", 10, 60),
    ]:
...
print("=" * 85)
print(
    f"{'Market':<10} {'Strategy':<20} {'Return':>12} {'Sharpe':>10} {'MDD':>10} {'vs BTC':>12}"
)
print("-" * 85)
for market, strat, r in results:
    print(
        f"{market:<10} {strat:<20} {r['return']*100:>+11.1f}% {r['sharpe']:>10.2f} {r['mdd']*100:>9.1f}% {(r['return']-r['btc_ret'])*100:>+11.1f}%p"
    )
...
for market, strat, r in results:
    ...
    diff = correct - biased
    print(
        f"{market:<10} {strat:<20} {correct:>+11.1f}% {biased:>+11.0f}% {diff:>+11.0f}%p"
    )
