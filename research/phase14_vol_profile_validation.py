...
def run_portfolio_oos(all_data, strat_fn, max_positions=10, test_bars=720,
                      exit_params=None, slippage=0.0003, position_scale=1.0):
...
def main():
    ...
    for scale in [1.0, 3.0, 5.0]:
        r = run_portfolio_oos(all_data, vol_profile_strategy, max_positions=10,
                              test_bars=720, exit_params=exit_params, position_scale=scale)
        if r:
    ...
    for slip_name, slip_val in [('0.02%', 0.0002), ('0.03%', 0.0003),
        ...
        r = run_portfolio_oos(all_data, vol_profile_strategy, max_positions=10,
                              ...
                              slippage=slip_val, position_scale=5.0)
        if r:
            c, p = check_criteria(r)
        ...
        for regime in ['bull', 'bear', 'sideways']:
            ...
            if all_trades:
                ...
                regime_results[regime] = {
                    'count': count, 'pct': pct, 'trades': len(all_trades),
                    'win_rate': wr, 'profit_factor': pf, 'net': net
                }
    ...
    if main_result and main_result.get('all_trades'):
        ...
        mc_stats = {
            ...
            'worst': mc_returns[0], 'best': mc_returns[-1],
        }
    ...
    param_variants = {
        ...
        'lookback_38':   {'lookback': 38, 'vwap_mult': 1.01, 'ema_fast': 50, 'ema_slow': 200},
        'lookback_58':   {'lookback': 58, 'vwap_mult': 1.01, 'ema_fast': 50, 'ema_slow': 200},
        'vwap_0.808':    {'lookback': 48, 'vwap_mult': 1.008, 'ema_fast': 50, 'ema_slow': 200},
        'vwap_1.012':    {'lookback': 48, 'vwap_mult': 1.012, 'ema_fast': 50, 'ema_slow': 200},
        'ema_fast_40':   {'lookback': 48, 'vwap_mult': 1.01, 'ema_fast': 40, 'ema_slow': 200},
        'ema_fast_60':   {'lookback': 48, 'vwap_mult': 1.01, 'ema_fast': 60, 'ema_slow': 200},
        'ema_slow_160':  {'lookback': 48, 'vwap_mult': 1.01, 'ema_fast': 50, 'ema_slow': 160},
        'ema_slow_240':  {'lookback': 48, 'vwap_mult': 1.01, 'ema_fast': 50, 'ema_slow': 240},
    }
    ...
    for name, params in param_variants.items():
        ...
        r = run_portfolio_oos(all_data, make_strat(), max_positions=10,
                              test_bars=720, exit_params=exit_params, position_scale=5.0)
        if r:
            c, p = check_criteria(r)
    ...
    baseline_r = run_portfolio_oos(all_data, baseline_strategy, max_positions=10,
                                   test_bars=720, exit_params=exit_params, position_scale=5.0)
    if baseline_r:
        c, p = check_criteria(baseline_r)
        print(f"\n  Baseline Dual MA (5x):")
        print(f"    [{p}/6] Sharpe={baseline_r['sharpe']:.2f}  Ret={baseline_r['total_return']*100:+.1f}%  "
    ...
    if main_result:
        ...
        print(f"\n  Vol Profile (5x):")
        print(f"    [{p_main}/6] Sharpe={main_result['sharpe']:.2f}  Ret={main_result['total_return']*100:+.1f}%  "
    ...
    if baseline_r and main_result and baseline_r.get('period_returns') and main_result.get('period_returns'):
...
""")
    ...
    report = {
        ...
        'config': {
            'lookback': 48, 'vwap_mult': 1.01,
            'ema_fast': 50, 'ema_slow': 200,
            'max_positions': 10, 'rebalance_bars': 720,
            'exit': {'max_bars': 72, 'atr_stop': 3.0, 'profit_target_atr': 8.0},
        },
        'results_by_scale': {
            str(k): {key: float(v) if isinstance(v, (float, np.floating)) else v
        ...
        'regime_results': {
            k: {key: float(val) if isinstance(val, (float, np.floating)) else val
