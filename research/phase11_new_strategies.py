...
def simulate_daily(
    prices, signals, position_pct=0.05, stop_pct=0.10, max_hold_days=30, slippage=0.001
):
...
def run_wfa_daily(
    ...
    while i + test_days <= len(prices):
        ...
        trades, ret = simulate_daily(
            test_prices, test_signals, position_pct, stop_pct, max_hold, slippage
        )
    ...
    for z_thresh in [1.5, 2.0, 2.5]:
        for lookback in [30, 60, 90]:
            ...
            for symbol in symbols_with_funding:
                ...
                daily_funding = (
                    funding.groupby("date")["fundingRate"].mean().reset_index()
                )
                ...
                merged = price_df.merge(
                    daily_funding[["date", "fundingRate"]], on="date", how="left"
                )
                merged["fundingRate"] = merged["fundingRate"].ffill()
                merged = merged.dropna(subset=["fundingRate", "close"]).reset_index(
                    drop=True
                )
                ...
                # Signals: contrarian
                signals = np.where(
                    z_score > z_thresh, -1, np.where(z_score < -z_thresh, 1, 0)
                )
            ...
            if all_trades:
                ...
                print(
                    f"  {config}: [{passed}/6] Sharpe={metrics['sharpe']:.2f} "
...
def strategy_2_fear_greed():
    ...
    for fear_level, greed_level in [(15, 85), (20, 80), (25, 75), (30, 70)]:
        for smoothing in [1, 7]:
            ...
            signals = np.where(
                fgi_val < fear_level, 1, np.where(fgi_val > greed_level, -1, 0)
            )
...
def strategy_3_macro_regime():
    ...
    merged = merged.merge(
        dxy[["date", "close"]].rename(columns={"close": "dxy"}), on="date", how="left"
    )
    merged = merged.merge(
    ...
    for dxy_ma in [50, 100]:
        for vix_thresh in [20, 25]:
            ...
            signals = np.where(
                dxy_below & vix_low, 1, np.where(~dxy_below & ~vix_low, -1, 0)
            )
        ...
        for top_n in [3, 5]:
            ...
            while i + rebal_days <= len(oos_dates):
                ...
                for s in alt_data:
                    try:
                        start_date = (
                            lookback_dates[0] if lookback_dates else period_dates[0]
                        )
                        end_date = period_dates[0]
                        if (
                            start_date in alt_prices[s].index
                            and end_date in alt_prices[s].index
                        ):
                            alt_ret = (
                                alt_prices[s][end_date] / alt_prices[s][start_date] - 1
                            )
                ...
                for s in selected:
                    try:
                        ...
                        ret = (
                            (end_p / start_p - 1) / top_n * 0.05
                        )  # equal weight, 5% total
...
def strategy_6_kimchi_premium():
    ...
    merged = merged.merge(
        upbit_btc[["date", "close"]].rename(columns={"close": "upbit_krw"}),
        on="date",
        how="inner",
    )
    merged = merged.merge(
        btc_futures[["date", "close"]].rename(columns={"close": "futures"}),
        on="date",
        how="inner",
    )
    ...
    for lookback in [30, 60]:
        for z_thresh in [1.5, 2.0]:
            ...
            signals = np.where(z > z_thresh, -1, np.where(z < -z_thresh, 1, 0))
...
def strategy_7_grid():
    ...
    for grid_pct in [0.01, 0.015, 0.02]:
        for adx_thresh in [20, 25]:
            ...
            plus_di = (
                pd.Series(plus_dm).rolling(n).mean()
                / (pd.Series(tr).rolling(n).mean() + 1e-10)
                * 100
            )
            minus_di = (
                pd.Series(minus_dm).rolling(n).mean()
                / (pd.Series(tr).rolling(n).mean() + 1e-10)
                * 100
            )
            ...
            for i in range(split, len(close)):
                ...
                if price_change < -grid_pct and position <= 0:
                    ...
                    if position == -1:
                        pnl = (
                            -1 * (close[i] - last_trade_price) / last_trade_price * 0.02
                        )
                        pnl -= COMMISSION * 0.02 * 2
                        trades.append(pnl)
                        capital += pnl
...
def strategy_8_composite_sentiment():
    ...
    if not trends.empty:
        ...
        trends_col = (
            [c for c in trends.columns if c != "datetime"][0]
            if len(trends.columns) > 1
            else None
        )
        if trends_col:
            trends["date"] = trends["datetime"].dt.date
            merged = merged.merge(
                trends[["date", trends_col]].rename(columns={trends_col: "gtrends"}),
                on="date",
                how="left",
            )
    ...
    for fear_thresh in [20, 25]:
        ...
        gt_norm = (
            (gtrends - gtrends.min()) / (gtrends.max() - gtrends.min() + 1e-10) * 100
        )
        ...
        signals = np.where(
            composite < fear_thresh, 1, np.where(composite > (100 - fear_thresh), -1, 0)
        )
...
def main():
    ...
    best_per_strategy.sort(
        key=lambda x: (-x[2], -x[3].get("sharpe", 0) if isinstance(x[3], dict) else 0)
    )
    ...
    for strat, config, passed, metrics in best_per_strategy:
        if isinstance(metrics, dict) and metrics:
            ...
            print(
                f"         Sharpe={metrics.get('sharpe',0):.2f}  Ret={metrics.get('total_return',0)*100:+.1f}%  "
