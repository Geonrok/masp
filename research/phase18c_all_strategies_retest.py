...
def main():
    ...
    print(
        f"Testing {len(strategies)} strategies at 5x scale, OOS>16000 (18 windows, ~2yr)\n"
    )

    header = (
        f"{'Strategy':<22} {'Orig':>4} {'Rev':>4} {'Sharpe':>7} {'Return':>8} "
    ...
    for name, cfg in strategies.items():
        ...
        if r:
            ...
            print(
                f"  {name:<20} [{p_orig}/6] [{p_rev}/6] {r['sharpe']:>6.2f} "
    ...
    if rev_pass:
        for n, p, r in sorted(rev_pass, key=lambda x: -x[2]["sharpe"]):
            print(
                f"  PASS: {n} Sharpe={r['sharpe']:.2f} Ret={r['total_return']*100:+.1f}% "
    ...
    for n, p, r in sorted(rev_5, key=lambda x: -x[2]["sharpe"]):
        print(
            f"  [{p}/6] {n} Sharpe={r['sharpe']:.2f} Ret={r['total_return']*100:+.1f}%"
        )
    ...
    for name, p_orig, p_rev, r in top:
        if r.get("period_returns"):
            ...
            print(f"\n  {name} [{p_rev}/6 rev]:")
            print(
                f"    First half ({len(first_half)}W): Ret={( r1-1)*100:+.1f}% WinRate={w1:.0f}%"
            )
            print(
                f"    Second half ({len(second_half)}W): Ret={(r2-1)*100:+.1f}% WinRate={w2:.0f}%"
            )
