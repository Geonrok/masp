...
if not impl_upbit.empty:
    print(f"  Upbit  - Sharpe: {impl_upbit['sharpe'].values[0]:.3f}, MDD: {impl_upbit['mdd'].values[0]*100:.1f}%, Return: {impl_upbit['total_return'].values[0]*100:.0f}%")
if not impl_bithumb.empty:
    print(f"  Bithumb - Sharpe: {impl_bithumb['sharpe'].values[0]:.3f}, MDD: {impl_bithumb['mdd'].values[0]*100:.1f}%, Return: {impl_bithumb['total_return'].values[0]*100:.0f}%")

print("\n[VALIDATED] KAMA=10, TSMOM=60, Gate=30:")
if not valid_upbit.empty:
    print(f"  Upbit  - Sharpe: {valid_upbit['sharpe'].values[0]:.3f}, MDD: {valid_upbit['mdd'].values[0]*100:.1f}%, Return: {valid_upbit['total_return'].values[0]*100:.0f}%")
if not valid_bithumb.empty:
    print(f"  Bithumb - Sharpe: {valid_bithumb['sharpe'].values[0]:.3f}, MDD: {valid_bithumb['mdd'].values[0]*100:.1f}%, Return: {valid_bithumb['total_return'].values[0]*100:.0f}%")
...
dsr_impl, p_impl, exp_max = deflated_sharpe_ratio(impl_sharpe, estimated_total_tests, years)
...
dsr_valid, p_valid, _ = deflated_sharpe_ratio(valid_sharpe_holdout, estimated_total_tests, years)
...
print(f"[VALIDATED] Holdout Sharpe {valid_sharpe_holdout:.2f}: {valid_percentile:.1f}th percentile")
...
if valid_sharpe_holdout > 2:
    score_valid += 1
print(f"[1/5] In-Sample Sharpe > 2: IMPL={'[PASS]' if impl_sharpe > 2 else '[FAIL]'}, VALID={'[PASS]' if valid_sharpe_holdout > 2 else '[FAIL]'}")
...
if p_valid < 0.05:
    score_valid += 1
print(f"[2/5] Deflated Sharpe p < 0.05: IMPL={'[PASS]' if p_impl < 0.05 else '[FAIL]'}, VALID={'[PASS]' if p_valid < 0.05 else '[FAIL]'}")
...
if valid_percentile > 95:
    score_valid += 1
print(f"[3/5] Monte Carlo > 95%: IMPL={'[PASS]' if impl_percentile > 95 else '[FAIL]'}, VALID={'[PASS]' if valid_percentile > 95 else '[FAIL]'}")
