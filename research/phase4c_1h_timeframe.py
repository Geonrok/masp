...
def strat_tsmom_ema_filter(df, lookback=168, ema_period=200):
    ...
    filtered = np.where((sig > 0) & (close > ema), 1,
               np.where((sig < 0) & (close < ema), -1, 0))
    return filtered
...
def strat_breakout_1h(df, lookback=48, ema_period=200):
    ...
    signals = np.where((close > upper) & (close > ema), 1,
...
def strat_mean_reversion_1h(df, bb_window=48, bb_std=2.5, rsi_window=14):
    ...
    signals = np.where((close < lower) & (rsi < 25), 1,
              np.where((close > upper) & (rsi > 75), -1, 0))
    return signals
...
def strat_vol_breakout_1h(df, lookback=24, atr_mult=1.0):
    ...
    signals = np.where((close > open_p + atr * atr_mult) & (close > ema200), 1,
...
def strat_keltner_breakout(df, ema_period=48, atr_period=14, atr_mult=2.0):
    ...
    signals = np.where(close > upper, 1,
...
# =============================================================================
def portfolio_wf(symbols, strat_func, strat_name,
                 train_bars=4320, test_bars=720,  # 180d train, 30d test (1h)
                 target_vol=0.10, max_positions=5,
                 atr_stop=0.0, trailing_atr=0.0, max_hold=72):
    ...
    while i + test_bars <= min_len:
        ...
        for symbol, vol in selected:
            ...
            r = simulate(test_df, test_sigs, position_pct, max_hold, atr_stop, trailing_atr)
...
def main():
    ...
    for strat_name, strat_func, _ in strategies:
        for stop_name, atr_s, trail_s, max_h in stop_configs:
            ...
            r = portfolio_wf(symbols, strat_func, key,
                             target_vol=0.10, max_positions=5,
                             atr_stop=atr_s, trailing_atr=trail_s, max_hold=max_h)
    ...
    promising_strats = [s for s in strategies if any(
    ...
    for strat_name, strat_func, _ in promising_strats:
        for stop_name, atr_s, trail_s, max_h in [('no_stop', 0, 0, 72), ('trail_3atr', 0, 3.0, 72)]:
    ...
    ranked = sorted(all_results, key=lambda x: (x['criteria_passed'], x['sharpe']), reverse=True)

    for rank, r in enumerate(ranked[:20], 1):
        marker = "***" if r['criteria_passed'] >= 5 else ("**" if r['criteria_passed'] >= 4 else "")
        print(f"  {rank:2d}. {r['name']:<40} [{r['criteria_passed']}/6]  "
    ...
    if ranked:
        ...
        imp = "IMPROVED" if best['criteria_passed'] > 4 or best['sharpe'] > 0.64 else "NOT IMPROVED"
        print(f"  Result: {imp}")
