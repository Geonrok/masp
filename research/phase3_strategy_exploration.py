...
# =============================================================================
def backtest_walk_forward(df: pd.DataFrame, signal_func, train_bars=1080, test_bars=180,
                          position_pct=0.02, max_bars_hold=48, **kwargs):
    """
...
def simulate_trades(df: pd.DataFrame, signals: pd.Series, position_pct=0.02, max_bars=48):
    ...
    for i in range(len(df)):
        ...
        if position != 0:
            ...
            if should_exit:
                ...
                pnl -= FUNDING_PER_8H * bars_held / 2 * position_pct  # funding (4h = half of 8h)
                trades.append(pnl)
                capital += pnl
                position = 0
...
def strategy_vol_breakout(train_df, test_df, atr_mult=1.5, lookback=24):
    ...
    signals = np.where(close > prev_close + atr * atr_mult, 1,
...
def strategy_funding_carry(train_df, test_df, symbol='', threshold=0.0003):
    ...
    try:
        ...
        signals = np.where(fr > threshold, -1,
                  np.where(fr < -threshold, 1, 0))
...
# =============================================================================
def strategy_funding_reversal(train_df, test_df, symbol='', lookback=24, zscore_threshold=2.0):
    ...
    try:
        ...
        combined_aligned['fr'] = fr_df[fr_col].reindex(combined_aligned.index, method='ffill')
        fr = combined_aligned['fr'].fillna(0)

        # Z-score of funding rate
        fr_zscore = (fr - fr.rolling(lookback).mean()) / (fr.rolling(lookback).std() + 1e-10)

        # Fade extremes
        signals = np.where(fr_zscore > zscore_threshold, -1,
...
def main():
    ...
    strategies = {
        '3.1a_tsmom_42': ('Momentum: TSMOM(42)', lambda t, s: strategy_tsmom(t, s, lookback=42)),
        '3.1b_tsmom_84': ('Momentum: TSMOM(84)', lambda t, s: strategy_tsmom(t, s, lookback=84)),
        '3.1c_dual_mom': ('Momentum: Dual', lambda t, s: strategy_dual_momentum(t, s, lookback=42)),
        '3.1d_kama_mom': ('Momentum: KAMA', lambda t, s: strategy_kama_momentum(t, s, kama_period=24)),
        '3.2a_bb_rev': ('MeanRev: Bollinger', lambda t, s: strategy_bb_reversion(t, s, window=24)),
        '3.2b_rsi_rev': ('MeanRev: RSI', lambda t, s: strategy_rsi_reversion(t, s, window=14)),
        '3.3a_donchian': ('Breakout: Donchian(25)', lambda t, s: strategy_donchian_breakout(t, s, lookback=25)),
        '3.3b_vol_break': ('Breakout: Volatility', lambda t, s: strategy_vol_breakout(t, s, atr_mult=1.5)),
        '3.7a_multi_factor': ('Hybrid: Multi-Factor', lambda t, s: strategy_multi_factor(t, s)),
    }
    ...
    for strat_key, (strat_name, strat_func) in strategies.items():
        ...
        for symbol in symbols:
            ...
            print(f"  {symbol:<12} [{status}] Ret={ret*100:+.1f}%  Sharpe={sharpe:.2f}  Trades={trades}")

        # Summary for this strategy
        profitable_count = sum(1 for r in strat_results.values() if r.get('profitable', False))
        ...
        all_results[strat_key] = {
            ...
            'profitable_pct': profitable_count / len(strat_results) * 100 if strat_results else 0,
            'avg_return': avg_return,
            'avg_sharpe': avg_sharpe,
            'avg_trades': avg_trades,
            'symbol_results': {s: {k: v for k, v in r.items() if k != 'period_returns'}
        ...
        print(f"  Avg Return: {avg_return*100:+.1f}%  Avg Sharpe: {avg_sharpe:.2f}  Avg Trades: {avg_trades:.0f}")

    # Test funding-based strategies on symbols with funding data
    funding_strategies = {
        '3.4a_funding_carry': ('Carry: Funding', lambda t, s, sym='': strategy_funding_carry(t, s, symbol=sym)),
        '3.5a_funding_rev': ('Sentiment: Funding Reversal', lambda t, s, sym='': strategy_funding_reversal(t, s, symbol=sym)),
    }
    ...
    for strat_key, (strat_name, strat_func) in funding_strategies.items():
        ...
        for symbol in symbols:
            ...
            result = backtest_walk_forward(df, lambda t, s: strat_func(t, s, sym=symbol))
            strat_results[symbol] = result
    ...
    ranked = sorted(all_results.items(), key=lambda x: x[1]['profitable_pct'], reverse=True)
    ...
    for rank, (key, res) in enumerate(ranked, 1):
        ...
        print(f"  {rank}. {marker} {res['name']:<30} {res['profitable_pct']:.0f}% profitable  "
    ...
    report = {
        ...
        'results': {k: {kk: vv for kk, vv in v.items() if kk != 'symbol_results'}
                   for k, v in all_results.items()},
        'detailed_results': all_results,
        'ranking': [{'rank': i+1, 'strategy': k, 'name': v['name'],
                     'profitable_pct': v['profitable_pct'], 'avg_return': v['avg_return'],
    ...
    state['next_actions'] = ['4.1: In-Sample backtest top strategies', '4.2: Out-of-Sample validation']
