...
def run_portfolio_with_regime(all_data, btc_df, strat_fn,
                               min_oos_bars=16000, max_positions=10,
                               test_bars=720, position_scale=5.0,
                               train_bars=4320, exit_params=None):
...
def main():
    ...
    strategies = {
        'dual_ma': (strat_dual_ma, {'max_bars': 72, 'atr_stop': 3.0, 'profit_target_atr': 8.0}),
        'vol_profile': (strat_vol_profile, {'max_bars': 72, 'atr_stop': 3.0, 'profit_target_atr': 8.0}),
        'obv_trend': (strat_obv_trend, {'max_bars': 72, 'atr_stop': 3.0, 'profit_target_atr': 8.0}),
        'mfi_60': (lambda df: strat_mfi_breakout(df, mfi_thresh=60), {'max_bars': 72, 'atr_stop': 3.0, 'profit_target_atr': 8.0}),
        'vwap_bkout': (strat_vwap_breakout, {'max_bars': 72, 'atr_stop': 3.0, 'profit_target_atr': 8.0}),
        'momentum': (strat_momentum_filter, {'max_bars': 72, 'atr_stop': 3.0, 'profit_target_atr': 8.0}),
        'dm_wide': (strat_dual_ma, {'max_bars': 96, 'atr_stop': 4.0, 'profit_target_atr': 10.0}),
    }

    all_results = {}
    for name, (fn, exit_p) in strategies.items():
        windows = run_portfolio_with_regime(all_data, btc_df, fn,
                                             min_oos_bars=16000,
                                             exit_params=exit_p)
    ...
    if all_results:
        ...
        print(f"\n  Window BTC returns:")
        for w in sample:
            print(f"    W{w['window_idx']:>2}: BTC {w['btc_return']*100:>+6.1f}% → {w['regime']}")
    ...
    for regime in ['BULL', 'BEAR', 'SIDEWAYS', 'ALL']:
        print(f"\n--- {regime} ---")
        print(f"  {'Strategy':<15} {'Return':>8} {'AvgPnL':>8} {'WinRate':>8} {'Windows':>8} {'MaxLoss':>8}")
        ...
        for name, windows in all_results.items():
            ...
            print(f"  {name:<15} {total_ret*100:>+7.1f}% {avg_pnl*100:>+7.2f}% "
                  f"{win_rate:>7.0f}% {len(rw):>8} {max_loss*100:>+7.2f}%")

            regime_scores.append((name, total_ret, avg_pnl, win_rate, max_loss, len(rw)))
    ...
    for name in strat_names:
        ...
        print(f"  {name:<15} {r.get('BULL','?'):>6} {r.get('BEAR','?'):>6} "
    ...
    for i, (name, score, r, ret) in enumerate(composite):
        neg_regimes = [reg for reg in ['BULL', 'BEAR', 'SIDEWAYS'] if ret.get(reg, 0) < 0]
        neg_str = f" WARNING NEGATIVE in: {', '.join(neg_regimes)}" if neg_regimes else ""
    ...
    for n_top in [2, 3]:
        ...
        print(f"    Overall: Sharpe={sharpe:.2f} Ret={total_ret*100:+.1f}% DD={dd*100:.1f}% WFA={wfa:.0f}%")
        ...
        for regime in ['BULL', 'BEAR', 'SIDEWAYS']:
            ...
            if regime_pnls:
                ...
                print(f"    {regime}: Ret={(r_total-1)*100:+.1f}% WinRate={r_wr:.0f}% Windows={len(regime_pnls)}")
