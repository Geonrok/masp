...
def task_1_2_analyze_date_ranges() -> Dict:
    ...
    result = {"date_ranges": date_ranges, "statistics": stats}

    print(f"Analyzed {len(date_ranges)} symbols")
    if stats:
        print(
            f"History range: {stats['min_history_days']} - {stats['max_history_days']} days"
        )
...
def task_1_3_check_data_quality() -> Dict:
    ...
    for symbol in major_symbols:
        ...
        if matches:
            try:
                ...
                gaps = (df["timestamp"].shift(-1) - df["timestamp"]) > pd.Timedelta(
                    hours=1
                )
                gap_count = gaps.sum()
                ...
                quality_report[symbol] = {
                    ...
                    "quality_score": (
                        "GOOD" if gap_count < 100 and extreme_moves < 50 else "CHECK"
                    ),
                }
            except Exception as e:
    ...
    # Summary
    good_count = sum(
        1 for v in quality_report.values() if v.get("quality_score") == "GOOD"
    )
...
def task_1_4_calculate_liquidity() -> Dict:
    ...
    result = {
        ...
        "liquidity_tiers": {
            "tier1_100m_plus": [
                s[0] for s in sorted_by_adv if s[1]["adv_notional_millions"] >= 100
            ],
            "tier2_10m_plus": [
                s[0] for s in sorted_by_adv if 10 <= s[1]["adv_notional_millions"] < 100
            ],
            "tier3_under_10m": [
                s[0] for s in sorted_by_adv if s[1]["adv_notional_millions"] < 10
            ],
        },
    ...
    print(
        f"  Tier 2 ($10-100M ADV): {len(result['liquidity_tiers']['tier2_10m_plus'])} symbols"
    )
    print(
        f"  Tier 3 (<$10M ADV): {len(result['liquidity_tiers']['tier3_under_10m'])} symbols"
    )
