...
def run_strategy_on_market(price_data: dict, strategy: dict, market_name: str) -> dict:
    ...
    result = run_backtest_volume_ranked(
        signal_data, common_dates, volume_data, INITIAL_CAPITAL, MAX_POSITIONS
    )
...
for strategy in STRATEGIES:
    ...
    for market_name in MARKETS.keys():
        ...
        if result:
            ...
            print(
                f"  {market_name}: Sharpe={result['sharpe']:.3f}, MDD={result['mdd']*100:.1f}%, Return={result['total_return']*100:.1f}%"
            )
...
for _, row in results_df.iterrows():
    print(
        f"{row['strategy']:<20} {row['market']:<10} {row['sharpe']:>8.3f} {row['mdd']*100:>7.1f}% {row['return']*100:>9.1f}%"
    )
