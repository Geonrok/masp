...
def run_backtest(data, kama_p, tsmom_p, symbol_filter=None):
    if symbol_filter:
        data = {k: v for k, v in data.items() if k.upper() in [s.upper() for s in symbol_filter]}
    ...
    for i, date in enumerate(dates_2025):
        ...
        if prev_positions:
            pnl = sum(portfolio * w * (prices_today[sym] - prev_p) / prev_p
                      for sym, (w, prev_p) in prev_positions.items() if sym in prices_today)
            portfolio += pnl
...
# Top coins
TOP_10 = ['BTC', 'BTCUSDT', 'ETH', 'ETHUSDT', 'SOL', 'SOLUSDT', 'XRP', 'XRPUSDT',
          'DOGE', 'DOGEUSDT', 'ADA', 'ADAUSDT', 'AVAX', 'AVAXUSDT', 'SHIB', 'SHIBUSDT',
          'DOT', 'DOTUSDT', 'LINK', 'LINKUSDT']

TOP_30 = TOP_10 + ['MATIC', 'MATICUSDT', 'TRX', 'TRXUSDT', 'UNI', 'UNIUSDT',
                   'ATOM', 'ATOMUSDT', 'LTC', 'LTCUSDT', 'BCH', 'BCHUSDT',
                   'NEAR', 'NEARUSDT', 'APT', 'APTUSDT', 'FIL', 'FILUSDT',
                   'ARB', 'ARBUSDT', 'OP', 'OPUSDT', 'IMX', 'IMXUSDT',
                   'INJ', 'INJUSDT', 'SUI', 'SUIUSDT', 'SEI', 'SEIUSDT']
