...
def strategy_tsmom_multifactor(df, lookback=84):
    ...
    signals = np.where(
        (score >= 2) & (tsmom > 0), 1, np.where((score <= -2) & (tsmom < 0), -1, 0)
    )
...
def portfolio_walk_forward(
    ...
    while i + test_bars <= min_len:
        ...
        if signal_func_name == "xsmom":
            ...
            xsmom_signals = strategy_xsmom(
                train_data, lookback=84, top_n=min(3, len(symbols) // 2)
            )
            ...
            for symbol, df in all_data.items():
                ...
                if np.isnan(vol) or vol == 0:
                    vol = 0.02
                position_pct = min(
                    target_vol / (vol * np.sqrt(6 * 365) + 1e-10) / len(symbols), 0.05
                )

                r = simulate_with_stops(
                    test, sig_array, position_pct, 48, atr_stop, trailing_stop
                )
            ...
            for symbol, signal, vol in selected:
                ...
                position_pct = min(
                    target_vol
                    / (vol * np.sqrt(6 * 365) + 1e-10)
                    / max(len(selected), 1),
                    0.05,
                )

                r = simulate_with_stops(
                    test, test_sigs, position_pct, 48, atr_stop, trailing_stop
                )
                period_pnl += r["total_return"]
                all_trades.extend(r["trades"])
...
def main():
    ...
    for strat_key, strat_name in strategies.items():
        for stop_key, stop_cfg in stop_configs.items():
            ...
            result = portfolio_walk_forward(
                symbols_10, strat_key, target_vol=0.10, max_positions=5, **stop_cfg
            )
            ...
            print(
                f"Sharpe={result['sharpe']:.2f}  Ret={result['total_return']*100:+.1f}%  "
    ...
    for strat_key in ["tsmom_regime", "tsmom_multifactor", "adaptive_tsmom", "xsmom"]:
        for stop_key in ["no_stop", "trail_3x"]:
            ...
            print(
                f"Testing: {strat_name} + {stop_key} (15% vol)...", end=" ", flush=True
            )

            result = portfolio_walk_forward(
                symbols_10, strat_key, target_vol=0.15, max_positions=5, **stop_cfg
            )
    ...
    for rank, (key, res) in enumerate(ranked[:15], 1):
        marker = (
            "***"
            if res["criteria_passed"] >= 5
            else ("**" if res["criteria_passed"] >= 4 else "")
        )
        print(
            f"  {rank:2d}. {key:<40} {res['criteria_passed']}/6  "
    ...
    if ranked:
        ...
        print(
            f"  Criteria: 4/6 → {best['criteria_passed']}/6 ({'improved' if best['criteria_passed'] > 4 else 'same or worse'})"
        )
