...
# 데이터 로드 함수들
# ============================================================
def load_ohlcv(exchange: str, timeframe: str = '1d', min_days: int = 100) -> Dict[str, pd.DataFrame]:
    """OHLCV 데이터 로드"""
...
# 필터 함수들
# ============================================================
def apply_funding_filter(signal: np.ndarray, dates: pd.DatetimeIndex,
                         funding_data: pd.DataFrame, threshold: float = 0.0005) -> np.ndarray:
    """
    Funding Rate 필터
...
def apply_oi_filter(signal: np.ndarray, dates: pd.DatetimeIndex,
                    oi_data: pd.DataFrame, lookback: int = 7) -> np.ndarray:
    """
    ...
    for i in range(lookback, len(oi_values)):
        if oi_values[i - lookback] > 0:
            oi_change[i] = (oi_values[i] - oi_values[i - lookback]) / oi_values[i - lookback]
...
def apply_lsratio_filter(signal: np.ndarray, dates: pd.DatetimeIndex,
                         ls_data: pd.DataFrame, threshold: float = 2.0) -> np.ndarray:
    """
    Long/Short Ratio 필터
...
def apply_fear_greed_filter(signal: np.ndarray, dates: pd.DatetimeIndex,
                            fg_data: pd.DataFrame,
                            extreme_fear: int = 20, extreme_greed: int = 80) -> np.ndarray:
    """
    Fear & Greed Index 필터
...
def apply_vix_filter(signal: np.ndarray, dates: pd.DatetimeIndex,
                     vix_data: pd.DataFrame, threshold: float = 30) -> np.ndarray:
    """
    VIX 필터
...
def apply_dxy_filter(signal: np.ndarray, dates: pd.DatetimeIndex,
                     dxy_data: pd.DataFrame, ma_period: int = 20) -> np.ndarray:
    """
    DXY (달러 인덱스) 필터
...
def backtest(
    ...
    for symbol, df in data.items():
        ...
        if filters.get('oi') and symbol in filters.get('oi_data', {}):
            signal = apply_oi_filter(
                signal, dates,
                filters['oi_data'].get(symbol)
            )
    ...
    for i, date in enumerate(all_dates):
        ...
        if targets:
            curr_val = cash + sum(
                s * prices_today.get(sym, 0)
                for sym, (s, _) in positions.items()
            )
    ...
    win_rate = winning_trades / (trades / 2) if trades > 0 else 0  # trades/2 = 매도 횟수
...
def main():
    ...
    print(f'총 테스트 수: {len(filter_combinations) * len(param_sets) * len(universes)}개')
    ...
    for universe in universes:
        ...
        for params in param_sets:
            for filter_combo in filter_combinations:
                ...
                if test_count % 50 == 0:
                    print(f'  진행: {test_count}/{total_tests} ({test_count/total_tests*100:.1f}%)')
                ...
                # 결과 저장
                filter_str = '+'.join([k for k, v in filter_combo.items() if v]) or 'base'
    ...
    for i, (_, row) in enumerate(top20.iterrows(), 1):
        print(f"  #{i}: {row['universe']} | KAMA{row['kama_p']}/TSMOM{row['tsmom_p']} | 필터: {row['filters']}")
        print(f"       수익률: {row['return']*100:.1f}% | 샤프: {row['sharpe']:.2f} | MDD: {row['mdd']*100:.1f}% | 승률: {row['win_rate']*100:.1f}%")
    ...
    # 유니버스별 평균
    print('\n[유니버스별 평균 성과]')
    univ_summary = df.groupby('universe').agg({
        'return': 'mean',
        'sharpe': 'mean',
        'mdd': 'mean',
        'win_rate': 'mean',
    ...
    for idx, row in univ_summary.iterrows():
        print(f"  {idx}: 수익률 {row['return']*100:.1f}% | 샤프 {row['sharpe']:.2f} | MDD {row['mdd']*100:.1f}% | 승률 {row['win_rate']*100:.1f}%")
    ...
    for univ in universes:
        ...
        if len(univ_df) > 0:
            ...
            print(f"  {univ}: {best['filters']} | 샤프 {best['sharpe']:.2f} | 수익률 {best['return']*100:.1f}%")
