...
def main():
    ...
    print(f'레버리지 ETF: {lev2x.index[0].strftime("%Y-%m-%d")} ~ {lev2x.index[-1].strftime("%Y-%m-%d")}')
    ...
    print(f'B&H: CAGR {bh_cagr*100:.1f}%, Sharpe {bh_sharpe:.3f}, MDD {bh_mdd*100:.1f}%')
    print()
    ...
    for name, func in strategies.items():
        ...
        try:
            ...
            print(f'  WF Train: {wf["train_sharpe"]:.3f}, Test: {wf["test_sharpe"]:.3f}')
            print(f'  최근 1년: {r1y*100:.1f}%')

            # 판정
            passed = (
                metrics['sharpe'] > 0.8 and
                metrics['mdd'] > -0.35 and
    ...
    if results:
        ...
        print()
        print(df[['strategy', 'cagr', 'sharpe', 'mdd', 'wf_test', 'recent_1y', 'verdict']].to_string(index=False))
        ...
        if len(passed) > 0:
            ...
            for _, row in passed.iterrows():
                print(f'  {row["strategy"]}: Sharpe {row["sharpe"]:.3f}, MDD {row["mdd"]*100:.1f}%, WF {row["wf_test"]:.3f}')
        else:
            print('실거래 가능 전략 없음')
            print()
