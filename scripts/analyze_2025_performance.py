...
def main():
    ...
    btc_2024 = btc_data[
        (btc_data.index >= "2024-01-01") & (btc_data.index < "2025-01-01")
    ]
    ...
    if len(btc_2024) > 0:
        ...
        print(
            f'  2024년: {btc_2024["close"].iloc[0]:,.0f} -> {btc_2024["close"].iloc[-1]:,.0f} ({ret_2024:+.1f}%)'
        )

    if len(btc_2025) > 0:
        ret_2025 = (btc_2025["close"].iloc[-1] / btc_2025["close"].iloc[0] - 1) * 100
        print(
            f'  2025년: {btc_2025["close"].iloc[0]:,.0f} -> {btc_2025["close"].iloc[-1]:,.0f} ({ret_2025:+.1f}%)'
        )
    ...
    atr = calc_atr(
        btc_data["high"].values, btc_data["low"].values, btc_data["close"].values
    )
    ...
    # OR_LOOSE 시그널
    btc_data["or_loose_kama5"] = (btc_data["kama5_signal"] | btc_data["tsmom90"]) & (
        prices > ma30
    )
    btc_data["or_loose_kama20"] = (btc_data["kama20_signal"] | btc_data["tsmom30"]) & (
        prices > ma30
    )
    ...
    for year in ["2024", "2025"]:
        ...
        if len(year_data) > 0:
            ...
            print(
                f"    KAMA5 시그널 전환: {kama5_changes}회 (평균 {len(year_data)/max(kama5_changes,1):.1f}일 유지)"
            )
            print(
                f"    KAMA20 시그널 전환: {kama20_changes}회 (평균 {len(year_data)/max(kama20_changes,1):.1f}일 유지)"
            )
    ...
    def backtest_period(
        data, btc_data, start_date, end_date, kama_p, tsmom_p, btc_ma_p, max_pos=10
    ):
        ...
        for i, date in enumerate(all_dates):
            ...
            active = [
                (s, vols_today.get(s, 0)) for s, sig in signals_today.items() if sig
            ]
    ...
    for kama_p, tsmom_p, ma_p, desc in params_to_test:
        ...
        print(
            f'KAMA{kama_p}/TSMOM{tsmom_p} ({desc:<12}) {result["return"]:>9.1f}% {result["sharpe"]:>8.2f}'
        )
    ...
    btc_data.loc[
        (btc_data["close"] > btc_data["ma50"]) & (btc_data["ma50"] > btc_data["ma200"]),
        "regime",
    ] = "bull"
    btc_data.loc[
        (btc_data["close"] < btc_data["ma50"]) & (btc_data["ma50"] < btc_data["ma200"]),
