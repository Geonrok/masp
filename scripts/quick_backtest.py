...
def backtest(data, btc_data, strategy, kama_p=5, tsmom_p=90, btc_ma_p=30, use_gate=True, max_pos=20):
    """바이어스 제거 백테스트"""
    if not data:
    ...
    for i, date in enumerate(all_dates):
        ...
        # 매수
        if targets:
            curr_val = cash + sum(s * prices_today.get(sym, 0) for sym, (s, _) in positions.items())
...
def main():
    ...
    strategies = ['buy_hold', 'kama_only', 'tsmom_only', 'kama_or_tsmom', 'kama_and_tsmom', 'or_loose']
    ...
    for exchange in exchanges:
        ...
        for universe in universes:
            ...
            if universe == 'btc_only':
                filtered = {k: v for k, v in data.items() if 'BTC' in k.upper() and 'DOWN' not in k.upper()}
                if not filtered:
                    continue
            ...
            for strategy in strategies:
                for kama_p, tsmom_p, btc_ma_p in params:
                    ...
                    result = backtest(
                        filtered, btc_data, strategy,
                        kama_p=kama_p, tsmom_p=tsmom_p, btc_ma_p=btc_ma_p,
                        use_gate=use_gate, max_pos=min(20, len(filtered))
                    )
        ...
        print(f'  {len([r for r in results if r["exchange"] == exchange])}개 테스트 완료')
    ...
    # 결과 저장
    output_file = Path('E:/투자/Multi-Asset Strategy Platform/outputs/comprehensive_results.csv')
    ...
    for ex in exchanges:
        ...
        print(f"  {ex.upper()}: {best['strategy']} ({best['universe']}) | "
    ...
    for idx, row in strat_summary.iterrows():
        print(f"  {idx}: 수익률 {row['return']*100:.1f}% | 샤프 {row['sharpe']:.2f} | MDD {row['mdd']*100:.1f}%")
    ...
    for i, (_, row) in enumerate(top10.iterrows(), 1):
        print(f"  #{i}: {row['exchange']} | {row['strategy']} | {row['universe']} | "
