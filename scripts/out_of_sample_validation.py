...
def backtest_period(data, btc_data, start_date, end_date, kama_p=5, tsmom_p=90, btc_ma_p=30, max_pos=10):
    """특정 기간에 대한 백테스트"""
...
def main():
    ...
    is_result = backtest_period(
        filtered, btc_data,
        pd.Timestamp('2017-01-01'), pd.Timestamp('2023-12-31')
    )

    oos_result = backtest_period(
        filtered, btc_data,
        pd.Timestamp('2024-01-01'), pd.Timestamp('2026-12-31')
    )
    ...
    for r in yearly_results:
        print(f'{r["year"]:<6} {r["return"]:>9.1f}% {r["sharpe"]:>8.2f} {r["mdd"]:>9.1f}% {r["days"]:>8}')

    # 통계
    df_yearly = pd.DataFrame(yearly_results)
    print('-' * 50)
    print(f'{"평균":<6} {df_yearly["return"].mean():>9.1f}% {df_yearly["sharpe"].mean():>8.2f} {df_yearly["mdd"].mean():>9.1f}%')
    print(f'{"표준편차":<6} {df_yearly["return"].std():>9.1f}% {df_yearly["sharpe"].std():>8.2f}')
    ...
    print('-' * 60)
    for r in wf_results:
        print(f'{r["test_year"]:<10} {r["train_sharpe"]:>10.2f} {r["test_sharpe"]:>10.2f} {r["train_return"]:>11.1f}% {r["test_return"]:>11.1f}%')
    ...
    if btc_data is not None:
        ...
        if len(btc_2024) > 0:
            btc_return = (btc_2024['close'].iloc[-1] / btc_2024['close'].iloc[0] - 1) * 100
            print(f'\n[2024년 이후]')
