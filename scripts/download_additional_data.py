    ...
    for i, ticker in enumerate(kosdaq_tickers):
        try:
            ...
            if len(df) > 100:
                ...
                df.to_csv(KOSDAQ_DIR / f'{ticker}.csv', index=False, encoding='utf-8-sig')
                downloaded += 1

            if (i + 1) % 100 == 0:
                print(f'  Progress: {i+1}/{len(kosdaq_tickers)} - Downloaded: {downloaded}')
                time.sleep(1)
    ...
    for i, ticker in enumerate(kospi_tickers):
        try:
            ...
            if len(df) > 100:
                ...
                df.to_csv(INVESTOR_DIR / f'{ticker}_investor.csv', index=False, encoding='utf-8-sig')
                downloaded += 1

            if (i + 1) % 50 == 0:
                print(f'  Progress: {i+1}/{len(kospi_tickers)} - Downloaded: {downloaded}')
                time.sleep(1)
...
try:
    ...
    dates_to_check = pd.date_range(start=START_DATE, end=END_DATE, freq='M').strftime('%Y%m%d').tolist()
    ...
    if all_fundamentals:
        ...
        fundamental_df.to_csv(FUNDAMENTAL_DIR / 'kospi_fundamentals.csv', index=False, encoding='utf-8-sig')
