...
class Backtester:
    ...
    def run(self, df: pd.DataFrame, signals: pd.DataFrame, symbol: str) -> Optional[BacktestResult]:
        if len(df) < 300:
            return None
...
def main():
    ...
    logger.info(f"Profitable (PF > 1.0): {len(profitable)} ({len(profitable)/len(results)*100:.1f}%)")
    logger.info(f"Losing (PF <= 1.0): {len(losing)} ({len(losing)/len(results)*100:.1f}%)")
    ...
    for r in sorted_by_pf:
        logger.info(f"  {r.symbol:12s} PF={r.profit_factor:5.2f} Ret={r.total_return:+8.1f}% "
    ...
    buckets = [
        ...
        ("PF 1.1-1.3 (Acceptable)", [r for r in results if 1.1 <= r.profit_factor < 1.3]),
    ...
    output_path = Path("E:/투자/Multi-Asset Strategy Platform/data/backtests/multitf_all_symbols_results.csv")
