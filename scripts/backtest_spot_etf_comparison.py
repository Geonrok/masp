...
for strat_name, strat_func in strategies.items():
    ...
    for lev in leverage_options:
        ...
        print(f"\n{strat_name} ({lev['name']}):")
        print(
            f"  Sharpe: {result['sharpe']:.3f}, CAGR: {result['cagr']*100:.1f}%, MDD: {result['mdd']*100:.1f}%"
        )
...
for strat_name in strategies.keys():
    ...
    print(f"  {'-'*50}")
    print(
        f"  {'Sharpe':<15} {etf_1x['sharpe']:.3f}{'':<10} {etf_2x['sharpe']:.3f}{'':<10} {'1x 우위' if etf_1x['sharpe'] > etf_2x['sharpe'] else '2x 우위'}"
    )
    print(
        f"  {'CAGR':<15} {etf_1x['cagr']*100:.1f}%{'':<9} {etf_2x['cagr']*100:.1f}%{'':<9} {'2x가 2배?' if etf_2x['cagr'] > etf_1x['cagr']*1.8 else '2배 미달'}"
    )
    print(
        f"  {'MDD':<15} {etf_1x['mdd']*100:.1f}%{'':<9} {etf_2x['mdd']*100:.1f}%{'':<9} {'2x MDD 심각' if etf_2x['mdd'] < -0.30 else 'OK'}"
    )
    print(
        f"  {'Calmar':<15} {etf_1x['calmar']:.3f}{'':<10} {etf_2x['calmar']:.3f}{'':<10} {'1x 우위' if etf_1x['calmar'] > etf_2x['calmar'] else '2x 우위'}"
    )
