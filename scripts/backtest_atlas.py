...
def load_and_calculate(symbol: str, start_date: str = '2023-01-01'):
    ...
    df['bbwp'] = df['bb_width'].rolling(252).apply(
        lambda x: percentileofscore(x, x.iloc[-1]) if len(x) >= 10 else 50, raw=False
    )
    ...
    for i in range(len(df)):
        ...
        if position is None:
            if long_entry.iloc[i]:
                position = {'side': 'LONG', 'entry': row['close'], 'entry_idx': i, 'high': row['close']}
            elif short_entry.iloc[i]:
                position = {'side': 'SHORT', 'entry': row['close'], 'entry_idx': i, 'low': row['close']}
        else:
            ...
            if position['side'] == 'LONG':
                ...
                pnl_pct = (row['close'] - position['entry']) / position['entry'] * leverage

                # Exit conditions
                if row['close'] < chandelier_stop or (bars_held >= 4 and pnl_pct < -0.0016):
                ...
                pnl_pct = (position['entry'] - row['close']) / position['entry'] * leverage
...
def main():
    ...
    for symbol in symbols:
        ...
        print(f'  Bars: {len(df)} ({df["datetime"].min().date()} ~ {df["datetime"].max().date()})')
        ...
        if trades:
            ...
            print(f'  Trades: {len(trades)} (L:{results[symbol]["long"]}, S:{results[symbol]["short"]})')
    ...
    if all_trades:
        ...
        for sym, r in results.items():
            print(f'{sym:<10} {r["return"]:>9.1f}% {r["mdd"]:>9.1f}% {r["trades"]:>8} {r["win_rate"]:>7.1f}%')

        # Recent trades
        print('\nRecent 10 Trades:')
        recent = all_df.tail(10)
        for _, t in recent.iterrows():
            print(f'  {t["time"].date()} {t["symbol"]} {t["side"]:5} {t["pnl_pct"]:+.2f}%')
