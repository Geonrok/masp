...
for strat_name, strat_func in strategies.items():
    ...
    if result_bias and result_no_bias:
        ...
        print(f"  {'-'*50}")
        print(
            f"  {'Sharpe':<15} {result_bias['sharpe']:.3f}{'':<8} {result_no_bias['sharpe']:.3f}{'':<8} {result_bias['sharpe'] - result_no_bias['sharpe']:+.3f}"
        )
        print(
            f"  {'CAGR':<15} {result_bias['annual_return']*100:.1f}%{'':<7} {result_no_bias['annual_return']*100:.1f}%{'':<7} {(result_bias['annual_return'] - result_no_bias['annual_return'])*100:+.1f}%p"
        )
        print(
            f"  {'MDD':<15} {result_bias['mdd']*100:.1f}%{'':<7} {result_no_bias['mdd']*100:.1f}%{'':<7} {(result_bias['mdd'] - result_no_bias['mdd'])*100:+.1f}%p"
        )

        if wf_bias and wf_no_bias:
            print(
                f"  {'WF Sharpe':<15} {wf_bias['sharpe']:.3f}{'':<8} {wf_no_bias['sharpe']:.3f}"
            )
...
for strat_name, strat_func in strategies.items():
    ...
    for leverage in [1.0, 2.0]:
        ...
        result = detailed_backtest(
            data_lev, signal, REALISTIC_COST * (1.5 if leverage > 1 else 1)
        )
...
for r in comparison_results:
    print(
        f"   {r['strategy']}: Sharpe {r['with_bias_sharpe']:.3f} → {r['no_bias_sharpe']:.3f} (차이: {r['sharpe_diff']:+.3f})"
    )
...
viable_2x = [
    ...
    if r["leverage"] == 2.0 and r["sharpe"] > 0.5 and r["mdd"] > -0.30
]
if viable_2x:
    print("   유효한 2x ETF 전략:")
    for v in viable_2x:
        print(
            f"   - {v['strategy']}: Sharpe {v['sharpe']:.3f}, MDD {v['mdd']*100:.1f}%"
        )
else:
...
viable_1x = [
    r
    for r in leverage_results
    if r["leverage"] == 1.0 and r["sharpe"] > 0.3 and r["mdd"] > -0.25
]
if viable_1x:
    print("\n   유효한 1x ETF 전략:")
    for v in viable_1x:
