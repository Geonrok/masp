...
class MultiFactorMLStrategy:
    ...
    def _calc_mf_scores(self, df: pd.DataFrame, btc_df: pd.DataFrame,
    ...
    def _create_ml_features(self, df: pd.DataFrame, mf_scores: pd.Series) -> pd.DataFrame:
        """Create ML features"""
        ...
        # Volume
        features['vol_zscore'] = (volume - volume.rolling(50).mean()) / (volume.rolling(50).std() + 1e-10)
    ...
    def backtest_period(self, df: pd.DataFrame, scores: pd.Series,
                        start_date, end_date, threshold: float = 4.0) -> Dict:
...
def run_walk_forward_validation(loader: DataLoader, symbols: List[str],
                                btc_df: pd.DataFrame, fear_greed: pd.DataFrame,
    ...
    for symbol in symbols:
        ...
        for p in periods:
            ...
            # Train backtest
            train_result = strategy.backtest_period(train_df, train_combined,
            ...
            # Test backtest
            test_result = strategy.backtest_period(test_df, test_combined,
    ...
    if all_train_results and all_test_results:
        ...
        train_profitable = sum(1 for r in all_train_results if r['pf'] > 1.0) / len(all_train_results) * 100
        test_profitable = sum(1 for r in all_test_results if r['pf'] > 1.0) / len(all_test_results) * 100

        logger.info(f"\n{'Metric':<25} {'Train':>15} {'Test (OOS)':>15}")
        logger.info("-" * 55)
        logger.info(f"{'Sample Count':<25} {len(all_train_results):>15} {len(all_test_results):>15}")
        logger.info(f"{'Profitable Rate':<25} {train_profitable:>14.1f}% {test_profitable:>14.1f}%")
        logger.info(f"{'Avg PF':<25} {np.mean(train_pfs):>15.2f} {np.mean(test_pfs):>15.2f}")
        logger.info(f"{'Median PF':<25} {np.median(train_pfs):>15.2f} {np.median(test_pfs):>15.2f}")
        logger.info(f"{'Avg Return':<25} {np.mean([r['ret'] for r in all_train_results]):>14.1f}% "
                   f"{np.mean([r['ret'] for r in all_test_results]):>14.1f}%")

        degradation = (np.mean(train_pfs) - np.mean(test_pfs)) / np.mean(train_pfs) * 100 if np.mean(train_pfs) > 0 else 0
...
def run_oos_validation(loader: DataLoader, symbols: List[str],
                       btc_df: pd.DataFrame, fear_greed: pd.DataFrame,
                       macro: pd.DataFrame) -> Dict:
    ...
    for symbol in symbols:
        ...
        train_ml_scores = strategy.predict(train_features)
        train_combined = train_mf + train_ml_scores * 0.8
        train_result = strategy.backtest_period(train_df, train_combined, TRAIN_START, TRAIN_END)
        ...
        test_ml_scores = strategy.predict(test_features)
        test_combined = test_mf + test_ml_scores * 0.8
        test_result = strategy.backtest_period(test_df, test_combined, TEST_START, TEST_END)
    ...
    logger.info(f"{'Avg PF':<25} {np.mean(train_pfs):>15.2f} {np.mean(test_pfs):>15.2f}")
    logger.info(f"{'Median PF':<25} {np.median(train_pfs):>15.2f} {np.median(test_pfs):>15.2f}")
    logger.info(f"{'Avg Return':<25} {df_comp['train_ret'].mean():>14.1f}% {df_comp['test_ret'].mean():>14.1f}%")
    logger.info(f"{'Avg Win Rate':<25} {df_comp['train_wr'].mean():>14.1f}% {df_comp['test_wr'].mean():>14.1f}%")
    ...
    if len(consistent) > 0:
        ...
        for _, r in top.iterrows():
            logger.info(f"  {r['symbol']:<14} Train PF={r['train_pf']:.2f} Test PF={r['test_pf']:.2f} "
...
def main():
    ...
    if oos_results:
        cons_rate = oos_results['consistent_count'] / oos_results['symbols_tested'] * 100
        criteria.append(("Consistent Symbols >= 20%", cons_rate >= 20, f"{cons_rate:.1f}%"))
