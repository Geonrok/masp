...
class Backtester:
    ...
    def run(self, df, scores, symbol):
        ...
        return Result(symbol, min(pf, 999), (capital/init-1)*100, wins/len(trades)*100, mdd, len(trades))
...
def main():
    ...
    # PF buckets
    logger.info("\nPF Distribution:")
    buckets = [(1.5, "PF>=1.5"), (1.3, "PF>=1.3"), (1.1, "PF>=1.1"), (1.0, "PF>=1.0"), (0, "PF<1.0")]
    ...
    for r in top:
        logger.info(f"  {r.symbol:14s} PF={r.pf:5.2f} Ret={r.ret:+8.1f}% WR={r.wr:4.0f}% MDD={r.mdd:6.1f}% Trades={r.trades:3d}")
    ...
    for r in recommended:
        logger.info(f"  {r.symbol:14s} PF={r.pf:5.2f} Ret={r.ret:+8.1f}% WR={r.wr:4.0f}% MDD={r.mdd:6.1f}%")

    # Save results
    results_df = pd.DataFrame([{
        'symbol': r.symbol, 'pf': r.pf, 'return': r.ret, 'wr': r.wr, 'mdd': r.mdd, 'trades': r.trades
    } for r in results])
    output_path = Path("E:/투자/Multi-Asset Strategy Platform/data/backtests/multifactor_results.csv")
