...
def main():
        ...
        for universe in universes:
            ...
            for strategy in strategies:
                for kama_p, tsmom_p, btc_ma_p in params:
                    result = backtest(filtered, btc_data, strategy, kama_p, tsmom_p, btc_ma_p, max_pos=min(10, len(filtered)))
                    results.append({
                        'exchange': exchange,
                        'universe': universe,
                        'strategy': strategy,
    ...
    output_file = Path('E:/투자/Multi-Asset Strategy Platform/outputs/spot_backtest_results.csv')
    ...
    for ex in exchanges:
        ...
        print(f"  {ex}: {best['strategy']} ({best['universe']}) | "
              f"수익률: {best['return']*100:.1f}% | 샤프: {best['sharpe']:.2f} | MDD: {best['mdd']*100:.1f}%")

    # 전략별 평균
    print('\n[전략별 평균 성과]')
    strat_summary = df.groupby('strategy').agg({'return': 'mean', 'sharpe': 'mean', 'mdd': 'mean'}).round(3)
    ...
    # 유니버스별 평균
    print('\n[유니버스별 평균 성과]')
    univ_summary = df.groupby('universe').agg({'return': 'mean', 'sharpe': 'mean', 'mdd': 'mean'}).round(3)
    ...
    for i, (_, row) in enumerate(top15.iterrows(), 1):
        print(f"  #{i}: {row['exchange']} | {row['strategy']} | {row['universe']} | KAMA{row['kama_p']}/TSMOM{row['tsmom_p']}")
