...
class DataCollector:
    ...
    async def collect_all(self, symbols: Optional[List[str]] = None):
        ...
        results = {"timestamp": datetime.now().isoformat(), "success": [], "failed": []}
        ...
        async with BinanceCollector() as binance:
            ...
            logger.info(
                f"  Completed: {len([s for s in results['success'] if s.startswith('OHLCV')])} symbols"
            )
            ...
            logger.info(
                f"  Completed: {len([s for s in results['success'] if s.startswith('FUNDING')])} symbols"
            )
        ...
        if not fg_df.empty:
            ...
            logger.info(
                f"  Latest: {fg_df['fear_greed'].iloc[-1]} ({fg_df['classification'].iloc[-1]})"
            )
        else:
        ...
        logger.info(
            f"  Completed: {len([s for s in results['success'] if s.startswith('MACRO')])} indicators"
        )
            ...
            "recommendation": (
                ...
                if total_score >= 4
                    ...
                    if total_score >= 2
                        ...
                        "NEUTRAL"
                        if total_score >= -2
                        else "SELL" if total_score >= -4 else "STRONG_SELL"
                    )
                )
            ),
        }
