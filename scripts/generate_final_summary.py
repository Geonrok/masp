...
if len(all_dfs) > 0:
    ...
    combined = combined.drop_duplicates(
        subset=["ticker", "strategy", "direction", "market"], keep="first"
    )
    ...
    print(
        f"  - KOSPI tickers: {combined[combined['market']=='KOSPI']['ticker'].nunique()}"
    )
    print(
        f"  - KOSDAQ tickers: {combined[combined['market']=='KOSDAQ']['ticker'].nunique()}"
    )
    ...
    print("=" * 80)
    strat_summary = (
        combined.groupby(["strategy", "direction"])
        .agg(
            {
                "cagr": "mean",
                "sharpe": "mean",
    ...
    print("=" * 80)
    top50 = combined.nlargest(50, "sharpe")[
        ["market", "ticker", "strategy", "direction", "cagr", "sharpe", "max_dd"]
    ]
    for _, row in top50.iterrows():
        print(
            f"{row['market']:>6} | {row['ticker']:>8} | {row['strategy']:<18} | {row['direction']:>5} | CAGR: {row['cagr']:>7.2%} | Sharpe: {row['sharpe']:>5.2f} | MDD: {row['max_dd']:>7.2%}"
        )
    ...
    print("=" * 80)
    kospi_top = combined[combined["market"] == "KOSPI"].nlargest(30, "sharpe")[
        ["ticker", "strategy", "direction", "cagr", "sharpe", "max_dd"]
    ]
    for _, row in kospi_top.iterrows():
        print(
            f"{row['ticker']:>8} | {row['strategy']:<18} | {row['direction']:>5} | CAGR: {row['cagr']:>7.2%} | Sharpe: {row['sharpe']:>5.2f} | MDD: {row['max_dd']:>7.2%}"
        )
    ...
    print("=" * 80)
    kosdaq_top = combined[combined["market"] == "KOSDAQ"].nlargest(30, "sharpe")[
        ["ticker", "strategy", "direction", "cagr", "sharpe", "max_dd"]
    ]
    ...
    combined.to_csv(
        RESULTS_DIR / f"FINAL_ALL_MARKETS_{timestamp}.csv",
        index=False,
        encoding="utf-8-sig",
    )
    tradable.to_csv(
        RESULTS_DIR / f"FINAL_TRADABLE_{timestamp}.csv",
        index=False,
        encoding="utf-8-sig",
    )
    strat_summary.to_csv(
        RESULTS_DIR / f"FINAL_STRATEGY_SUMMARY_{timestamp}.csv", encoding="utf-8-sig"
    )
