            ...
            if fp.exists():
                try:
                    ...
                    if "datetime" in df.columns:
                        ...
                        df["timestamp"] = pd.to_datetime(df["timestamp"])
                        df = df.set_index("timestamp")
                    cols = [c for c in df.columns if 'funding' in c.lower() or 'rate' in c.lower()]
                    if cols:
...
class Backtester:
    ...
    def run(self, df: pd.DataFrame, signals: pd.DataFrame, symbol: str, strategy: str) -> Optional[Result]:
        ...
        for i in range(1, len(df)):
            ...
            exit_sig = signals['exit'].iloc[i] if i < len(signals) and 'exit' in signals.columns else False

            if position:
                if exit_sig or (sig != 0 and sig != position.direction):
                    exit_p = price * (1 - self.slippage * position.direction)
                    pnl_pct = (exit_p / position.entry_price - 1) * position.direction - self.commission * 2
        ...
        if position:
            pnl_pct = (df['close'].iloc[-1] / position.entry_price - 1) * position.direction - self.commission * 2
        ...
        return Result(symbol, strategy, min(pf, 999), (capital/init-1)*100, wr, mdd, len(trades), sharpe)
...
class Strategy1_MultiTF:
    ...
    def generate(self, df_4h: pd.DataFrame, df_1d: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        for i in range(250, len(df)):
            ...
            if pd.isna(td): continue

            if pos == 1 and (t4h == -1 or td == -1):
                signals.iloc[i, 1] = True; pos = 0
            elif pos == -1 and (t4h == 1 or td == 1):
                signals.iloc[i, 1] = True; pos = 0

            if pos == 0:
                if t4h == 1 and td == 1:
                    signals.iloc[i, 0] = 1; pos = 1
                elif t4h == -1 and td == -1:
                    signals.iloc[i, 0] = -1; pos = -1
        return signals
            ...
            if atr_pct < 30:  # Low vol: mean reversion
                if pos == 0:
                    if c < df['bb_lower'].iloc[i]:
                        ...
                        signals.iloc[i, 0] = -1; pos = -1
                elif pos != 0 and abs(c - df['bb_mid'].iloc[i]) / df['bb_mid'].iloc[i] < 0.01:
                    signals.iloc[i, 1] = True; pos = 0
...
class Strategy5_FundingRate:
    ...
    def generate(self, df_4h: pd.DataFrame, funding: pd.DataFrame, **kwargs) -> pd.DataFrame:
        signals = pd.DataFrame(index=df_4h.index, columns=['signal', 'exit'])
        signals['signal'] = 0
        signals['exit'] = False
        ...
        for i in range(60, len(df)):
            if pos != 0:
                bars += 1
                if bars >= 18:
                    signals.iloc[i, 1] = True; pos = 0; bars = 0
                continue

            fund = df['funding_ma'].iloc[i]
            trend = df['trend'].iloc[i]

            if pd.isna(fund): continue

            if fund < -0.0005 and trend == 1:
                signals.iloc[i, 0] = 1; pos = 1; bars = 0
...
class Strategy6_BTCCorr:
    ...
    def generate(self, df_4h: pd.DataFrame, btc_df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        signals = pd.DataFrame(index=df_4h.index, columns=['signal', 'exit'])
        signals['signal'] = 0
        signals['exit'] = False
...
class Strategy7_Ensemble:
    ...
    def generate(self, df_4h: pd.DataFrame, btc_df: pd.DataFrame, fear_greed: pd.DataFrame, **kwargs) -> pd.DataFrame:
        signals = pd.DataFrame(index=df_4h.index, columns=['signal', 'exit'])
        signals['signal'] = 0
        signals['exit'] = False
        ...
        if not btc_df.empty:
            ...
            df['btc_sig'] = np.where(df['btc_mom'] > 0.05, 1, np.where(df['btc_mom'] < -0.05, -1, 0))
        else:
            df['btc_sig'] = 0
        ...
        df['long_score'] = ((df['rsi_sig']==1).astype(int) + (df['trend_sig']==1).astype(int) +
                           (df['btc_sig']==1).astype(int) + (df['fg_sig']==1).astype(int) +
                           (df['ema200_sig']==1).astype(int))
        df['short_score'] = ((df['rsi_sig']==-1).astype(int) + (df['trend_sig']==-1).astype(int) +
                            (df['btc_sig']==-1).astype(int) + (df['fg_sig']==-1).astype(int) +
                            (df['ema200_sig']==-1).astype(int))
...
class Strategy8_LongOnlyFear:
    ...
    def generate(self, df_4h: pd.DataFrame, fear_greed: pd.DataFrame, **kwargs) -> pd.DataFrame:
        signals = pd.DataFrame(index=df_4h.index, columns=['signal', 'exit'])
        signals['signal'] = 0
        signals['exit'] = False
    ...
    def generate(self, df_4h: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        for i in range(60, len(df)):
            ...
            if ema9_prev <= ema21_prev and ema9 > ema21 and df['close'].iloc[i] > ema50:
                if pos != 1:
                    if pos == -1: signals.iloc[i, 1] = True
                    signals.iloc[i, 0] = 1; pos = 1
            # Death cross
            elif ema9_prev >= ema21_prev and ema9 < ema21 and df['close'].iloc[i] < ema50:
                if pos != -1:
                    if pos == 1: signals.iloc[i, 1] = True
                    signals.iloc[i, 0] = -1; pos = -1
        return signals
...
def main():
    ...
    results: List[Result] = []
    strategy_stats = defaultdict(lambda: {'profitable': 0, 'total': 0, 'pfs': [], 'rets': []})
    ...
    for name in sorted(strategy_stats.keys()):
        ...
        logger.info(f"\n{name}:")
        logger.info(f"  Profitable: {stats['profitable']}/{stats['total']} ({prof_pct:.1f}%)")
    ...
    for i, s in enumerate(summary, 1):
        logger.info(f"  {i}. {s['strategy']}: {s['profitable']}/{s['total']} ({s['prof_pct']:.1f}%), "
    ...
    for r in top_symbols:
        logger.info(f"  {r.symbol:14s} {r.strategy:16s} PF={r.pf:5.2f} Ret={r.ret:+8.1f}% "
    ...
    symbol_profit_count = defaultdict(lambda: {'count': 0, 'strategies': [], 'avg_pf': []})
    for r in results:
        if r.pf > 1.0:
    ...
    for symbol, data in robust[:30]:
        ...
        logger.info(f"  {symbol:14s} Profitable in {data['count']:2d} strategies, Avg PF={avg_pf:.2f}")
    ...
    output_path = Path("E:/투자/Multi-Asset Strategy Platform/data/backtests/comprehensive_all_results.csv")
