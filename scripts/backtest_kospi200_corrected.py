...
print(f"  데이터: {len(daily_data)}일 ({daily_data.index.min().date()} ~ {daily_data.index.max().date()})")
...
for period in [5, 7, 10, 15, 20, 30, 50, 100, 200]:
    if len(daily_data) > period:
        ...
        daily_data[f'ema_{period}'] = daily_data['close'].ewm(span=period, adjust=False).mean()
...
def walk_forward_analysis(data, signal_func, cost, train_ratio=0.7, annual_factor=252):
    ...
    test_result = detailed_backtest(test_data, test_signal, cost, annual_factor, min_bars=50)
...
strategies = {
    ...
    'VIX_Declining': lambda d: ((d['vix'] < d['vix_sma_10']) & (d['vix_change_5d'] < -0.05)).astype(int),

    # 반도체 + 외국인 (Semicon은 T-1, Foreign은 한국 데이터)
    'Semicon_Foreign': lambda d: ((d['semicon'] > d['semicon_sma_20']) & (d['foreign_20d'] > 0)).astype(int),
    'Semicon_CCI': lambda d: ((d['semicon'] > d['semicon_sma_20']) & (d['cci_14'] > 0)).astype(int),

    # 외국인 기반 (한국 데이터 - shift 불필요)
    'Foreign_20d_Trend': lambda d: ((d['foreign_20d'] > 0) & (d['close'] > d['sma_50'])).astype(int),
    'Foreign_30d_SMA_100': lambda d: ((d['foreign_30d'] > 0) & (d['close'] > d['sma_100'])).astype(int),
...
for name, signal_func in strategies.items():
    ...
    if full_result:
        ...
        print(f"    Sharpe: {full_result['sharpe']:.3f}, CAGR: {full_result['annual_return']*100:.1f}%, MDD: {full_result['mdd']*100:.1f}%")
    ...
    if wf['test']:
        ...
        print(f"    WF Test: Sharpe {wf['test']['sharpe']:.3f}, CAGR {wf['test']['annual_return']*100:.1f}%")
...
# 유효 전략 필터
viable_strict = results_df[
    (results_df['sharpe'] > 1.0) &
    (results_df['mdd'] > -0.15)
]

viable_relaxed = results_df[
...
output = {
    ...
    'viable_relaxed': viable_relaxed.to_dict('records') if len(viable_relaxed) > 0 else [],
}
