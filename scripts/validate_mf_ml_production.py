...
def monte_carlo_simulation(
    trades: List[Dict], n_simulations: int = 10000, initial_capital: float = 10000
) -> Dict:
    ...
    return {
        ...
        "percentile_25": (np.percentile(final_capitals, 25) / initial_capital - 1)
        * 100,
        "percentile_75": (np.percentile(final_capitals, 75) / initial_capital - 1)
        * 100,
        "percentile_95": (np.percentile(final_capitals, 95) / initial_capital - 1)
        * 100,
        ...
        for i in range(10, len(period_df)):
            ...
            if position:
                ...
                if should_exit:
                    ...
                    cost = cost_model.calc_total_cost(
                        position["entry"], price, volatility, bars_held
                    )
...
def execution_lag_test(
    ...
    for i in range(50 + lag_bars, len(df)):
        ...
        if position:
            ...
            should_exit = (
                (position["dir"] == 1 and delayed_exit_signal < 0)
                or (position["dir"] == -1 and delayed_exit_signal > 0)
                or bars_held >= 36
            )
...
def main():
    ...
    for symbol in test_symbols:
        ...
        result = sequential_walk_forward(
            df, btc_df, fear_greed, macro, cost_model, retrain_period=180, symbol=symbol
        )
        if result:
    ...
    for symbol in stress_symbols:
        ...
        ml_scores = pd.Series(
            [ml_signal.predict(df, i) for i in range(len(df))], index=df.index
        )
        ...
        if stress_results:
            ...
            for name, result in stress_results.items():
                status = (
                    "PASS"
                    if result["return"] > -10
                    else "WARN" if result["return"] > -20 else "FAIL"
                )
                logger.info(
                    f"  {name:<20} [{status}] Ret={result['return']:+.1f}% "
    ...
    for symbol in ["BTCUSDT", "ETHUSDT", "SOLUSDT"]:
        ...
        for lag in [0, 1, 2, 3]:  # 0, 4, 8, 12시간 지연
            ...
            if result:
                ...
                logger.info(
                    f"  {lag:>8} {lag*4:>8}h {result['pf']:>8.2f} "
    ...
    if mc_result:
        ...
        logger.info(
            f"  [{'PASS' if c2 else 'FAIL'}] Monte Carlo 수익 확률 >= 60%: {mc_result['prob_profit']:.1f}%"
        )
        criteria.append(c2)
    ...
    if mc_result:
        ...
        logger.info(
            f"  [{'PASS' if c3 else 'FAIL'}] 최악 시나리오(5%) >= -20%: {mc_result['percentile_5']:.1f}%"
        )
        criteria.append(c3)
    ...
    if lag_results:
        ...
        if lag_4h:
            ...
            c5 = avg_ret_lag > 0
            logger.info(
                f"  [{'PASS' if c5 else 'FAIL'}] 4시간 지연 시 수익: {avg_ret_lag:+.1f}%"
            )
            criteria.append(c5)
    ...
    if all_trades:
        ...
        if wins and losses:
            ...
            logger.info(
                f"  [{'PASS' if c6 else 'FAIL'}] Kelly 기준 포지션 >= 10%: {kelly*100:.1f}%"
            )
            criteria.append(c6)
