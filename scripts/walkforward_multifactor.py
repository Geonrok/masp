...
def main():
    ...
    periods = generate_walk_forward_periods("2022-01-01", "2025-01-01", train_months=9, test_months=3)
    logger.info(f"Generated {len(periods)} walk-forward periods\n")

    for i, p in enumerate(periods):
        logger.info(f"  Period {i+1}: Train {p['train_start'].strftime('%Y-%m')} ~ {p['train_end'].strftime('%Y-%m')} | Test {p['test_start'].strftime('%Y-%m')} ~ {p['test_end'].strftime('%Y-%m')}")
    ...
    for symbol in all_symbols:
        ...
        if not df.empty and df.index.min() <= pd.Timestamp("2022-06-01") and len(df) >= 500:
    ...
    for symbol in valid_symbols:
        ...
        for i, p in enumerate(periods):
            train_result = backtester.run_period(df, scores, p['train_start'], p['train_end'])
            test_result = backtester.run_period(df, scores, p['test_start'], p['test_end'])
    ...
    logger.info("=" * 70)
    logger.info(f"{'Period':<10} {'Train Period':<25} {'Test Period':<25} {'Train PF':>10} {'Test PF':>10} {'Degrad':>10}")
    logger.info("-" * 90)
    ...
    for i, p in enumerate(periods):
        ...
        if train_pfs and test_pfs:
            ...
            logger.info(f"Period {i+1:<3} {train_str:<25} {test_str:<25} {avg_train:>10.2f} {avg_test:>10.2f} {degradation:>9.1f}%")
    ...
    if all_train_pfs and all_test_pfs:
        ...
        train_profitable = sum(1 for pf in all_train_pfs if pf > 1.0) / len(all_train_pfs) * 100
        test_profitable = sum(1 for pf in all_test_pfs if pf > 1.0) / len(all_test_pfs) * 100

        logger.info(f"Total symbol-period combinations: {len(all_train_pfs)}")
        logger.info(f"")
        logger.info(f"{'Metric':<25} {'In-Sample (Train)':>20} {'Out-of-Sample (Test)':>20}")
        logger.info("-" * 65)
        logger.info(f"{'Average PF':<25} {avg_train_pf:>20.2f} {avg_test_pf:>20.2f}")
        logger.info(f"{'Profitable Rate':<25} {train_profitable:>19.1f}% {test_profitable:>19.1f}%")
        logger.info(f"{'Median PF':<25} {np.median(all_train_pfs):>20.2f} {np.median(all_test_pfs):>20.2f}")
        logger.info(f"")
        ...
        criteria.append(("Degradation < 40%", degrad_check, f"{overall_degradation:.1f}%"))
        ...
        if period_test_avgs:
            consistency = np.std(period_test_avgs) / np.mean(period_test_avgs) if np.mean(period_test_avgs) > 0 else 999
            consistency_check = consistency < 0.5
            criteria.append(("Period Consistency (CV < 0.5)", consistency_check, f"CV={consistency:.2f}"))
    ...
    stable_symbols = [s for s in symbol_results if s['avg_test_pf'] > 1.0 and s['degradation'] < 30]
    stable_symbols = sorted(stable_symbols, key=lambda x: -x['avg_test_pf'])[:30]

    logger.info(f"{'Symbol':<14} {'Train PF':>10} {'Test PF':>10} {'Degradation':>12} {'Periods':>8}")
    logger.info("-" * 55)
    for s in stable_symbols:
        logger.info(f"{s['symbol']:<14} {s['avg_train_pf']:>10.2f} {s['avg_test_pf']:>10.2f} {s['degradation']:>11.1f}% {s['periods_tested']:>8}")
