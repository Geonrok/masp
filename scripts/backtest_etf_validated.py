...
def backtest_original_method(data, signal_series, leverage=1.0,
                            fee=0.00003, slippage=0.0002):
    """
...
def walk_forward_validation(data, signal_func, leverage=1.0, n_splits=11):
    ...
    return {
        ...
        'wf_ratio': np.mean(oos_sharpes) / np.mean(is_sharpes) if is_sharpes and np.mean(is_sharpes) > 0 else None,
    }
...
for strat_name in ['VIX_Below_SMA20', 'VIX_Declining', 'Composite_50_30_20']:
    ...
    print(f"  {'-'*60}")
    print(f"  {'Sharpe':<20} {etf_1x['sharpe']:.3f}{'':<8} {etf_2x['sharpe']:.3f}{'':<8} {'1x 우위' if etf_1x['sharpe'] > etf_2x['sharpe'] else '2x 우위'}")
    print(f"  {'CAGR':<20} {etf_1x['cagr']*100:.1f}%{'':<7} {etf_2x['cagr']*100:.1f}%{'':<7}")
    print(f"  {'MDD':<20} {etf_1x['mdd']*100:.1f}%{'':<7} {etf_2x['mdd']*100:.1f}%{'':<7} {'MDD 2배 이상' if etf_2x['mdd'] < etf_1x['mdd'] * 1.8 else 'OK'}")
    print(f"  {'Calmar (수익/위험)':<20} {etf_1x['calmar']:.3f}{'':<8} {etf_2x['calmar']:.3f}{'':<8} {'1x 우위' if etf_1x['calmar'] > etf_2x['calmar'] else '2x 우위'}")

    if etf_1x['wf_oos_sharpe'] and etf_2x['wf_oos_sharpe']:
        print(f"  {'WF OOS Sharpe':<20} {etf_1x['wf_oos_sharpe']:.3f}{'':<8} {etf_2x['wf_oos_sharpe']:.3f}")
...
if len(viable_2x) > 0:
    ...
    # 현재 결과에서 최선
    vix_1x = results_df[(results_df['strategy'] == 'VIX_Below_SMA20') & (results_df['leverage'] == 1.0)]
    if len(vix_1x) > 0:
        ...
        print(f"\n현재 백테스트 결과:")
        print(f"  VIX_Below_SMA20 (1x): Sharpe {vix_1x['sharpe']:.3f}, MDD {vix_1x['mdd']*100:.1f}%")
...
output = {
    ...
    'viable_1x_strict': viable_1x_strict.to_dict('records') if len(viable_1x_strict) > 0 else [],
    'viable_1x_relaxed': viable_1x_relaxed.to_dict('records') if len(viable_1x_relaxed) > 0 else [],
