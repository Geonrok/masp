...
class MultiFactorMLStrategy:
    ...
    def calc_mf_scores(self, df: pd.DataFrame, btc_df: pd.DataFrame,
    ...
    def backtest(self, df: pd.DataFrame, scores: pd.Series, threshold: float = 4.0) -> Dict:
        if len(df) < 50:
            return None
...
def run_walk_forward_paired(loader: DataLoader, symbols: List[str],
    ...
    logger.info(f"{'Profitable Count':<25} {train_profitable:>15} {test_profitable:>15}")
    logger.info(f"{'Profitable Rate':<25} {train_profitable/n*100:>14.1f}% {test_profitable/n*100:>14.1f}%")
    logger.info(f"{'Avg PF':<25} {train_pfs.mean():>15.2f} {test_pfs.mean():>15.2f}")
    logger.info(f"{'Median PF':<25} {train_pfs.median():>15.2f} {test_pfs.median():>15.2f}")
    logger.info(f"{'Avg Return':<25} {df_paired['train_ret'].mean():>14.1f}% {df_paired['test_ret'].mean():>14.1f}%")
    logger.info(f"{'Avg Win Rate':<25} {df_paired['train_wr'].mean():>14.1f}% {df_paired['test_wr'].mean():>14.1f}%")

    degradation = (train_pfs.mean() - test_pfs.mean()) / train_pfs.mean() * 100 if train_pfs.mean() > 0 else 0
    ...
    for period in df_paired['period'].unique():
        ...
        logger.info(f"  {period}: Train {train_prof:.0f}% → Test {test_prof:.0f}% ({len(subset)} samples)")
...
def run_oos_validation_paired(loader: DataLoader, symbols: List[str],
                              btc_df: pd.DataFrame, fear_greed: pd.DataFrame,
                              macro: pd.DataFrame) -> Dict:
...
def main():
    ...
    if oos_results:
        criteria.append(("OOS Profitable >= 50%", oos_results['test_profitable_rate'] >= 50,
                        f"{oos_results['test_profitable_rate']:.1f}%"))
        criteria.append(("OOS Avg PF >= 1.0", oos_results['test_avg_pf'] >= 1.0,
                        f"{oos_results['test_avg_pf']:.2f}"))
        criteria.append(("PF Degradation <= 50%", oos_results['degradation'] <= 50,
    ...
    if wf_results:
        criteria.append(("Walk-Forward Test >= 40%", wf_results['test_profitable_rate'] >= 40,
