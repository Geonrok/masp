...
def strategy_trend_follow(df: pd.DataFrame, fast: int = 20, slow: int = 50) -> pd.Series:
    """
    단순 이동평균 크로스
...
def strategy_volatility_breakout(df: pd.DataFrame, atr_mult: float = 1.5, atr_period: int = 14) -> pd.Series:
    """
    ATR 기반 변동성 돌파
...
def strategy_mean_reversion(df: pd.DataFrame, period: int = 20, std_mult: float = 2.0) -> pd.Series:
    """
    볼린저밴드 역추세
    - 하단 밴드 아래: Long (과매도)
    - 상단 밴드 위: Short (과매수)
...
def strategy_volume_spike(df: pd.DataFrame, vol_mult: float = 2.0, lookback: int = 20) -> pd.Series:
    """
    거래량 급증 + 가격 방향
    - 거래량 급증 + 상승: Long
    - 거래량 급증 + 하락: Short
...
def strategy_rsi_extreme(df: pd.DataFrame, period: int = 14, oversold: int = 30, overbought: int = 70) -> pd.Series:
    """
    RSI 극단값 역추세
    - RSI < oversold: Long
...
def strategy_dual_momentum(df: pd.DataFrame, btc_df: pd.DataFrame, lookback: int = 30) -> pd.Series:
    """
    이중 모멘텀
    - 절대 모멘텀: 본인 수익률 > 0
    - 상대 모멘텀: BTC 대비 우수
...
def backtest(df: pd.DataFrame, signals: pd.Series, cost: CostModel,
             position_pct: float = 0.2, leverage: float = 2.0,
             max_bars: int = 36) -> Dict:
...
def main():
    ...
    for name, res_list in results.items():
        ...
        logger.info(f"{name:<15} {n:>8} {profitable/n*100:>11.1f}% {avg_pf:>10.2f} "
    ...
    if summary:
        ...
        for _, r in df_best.sort_values('pf', ascending=False).iterrows():
            ...
            logger.info(f"[{status}] {r['symbol']:<10} {r['pf']:>8.2f} {r['ret']:>9.1f}% "
    ...
    if len(summary) >= 3:
        ...
        if combo_results:
            ...
            logger.info(f"  수익 심볼: {profitable}/{len(combo_results)} ({profitable/len(combo_results)*100:.1f}%)")
            ...
            for _, r in df_combo.sort_values('pf', ascending=False).head(10).iterrows():
                status = "PASS" if r['pf'] > 1.0 else "FAIL"
                logger.info(f"  [{status}] {r['symbol']:<12} PF={r['pf']:5.2f} Ret={r['ret']:+6.1f}%")
    ...
    if summary:
        ...
        if len(profitable_strategies) > 0:
            ...
            for _, s in profitable_strategies.iterrows():
                logger.info(f"  - {s['strategy']}: {s['profitable_pct']:.1f}% 수익, PF {s['avg_pf']:.2f}")
        else:
