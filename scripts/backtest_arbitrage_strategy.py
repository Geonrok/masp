...
class BasisArbitrage:
    ...
    def backtest(self, spot_df: pd.DataFrame, futures_df: pd.DataFrame,
                 funding_df: pd.DataFrame, symbol: str) -> Dict:
        ...
        if not funding_df.empty:
            funding = funding_df['funding_rate'].reindex(common_idx, method='ffill').fillna(0)
        else:
        ...
        for i in range(50, len(common_idx)):
            ...
            if position:
                ...
                should_exit = current_funding < 0 or current_basis < position['entry_basis'] * 0.3

                if should_exit:
                    # Close position
                    spot_pnl = (spot_price / position['spot_entry'] - 1) * position['size']
                    futures_pnl = (position['futures_entry'] / futures_price - 1) * position['size']
            ...
            if not position and current_funding > self.funding_threshold and current_basis > 0.1:
        ...
        if position:
            ...
            futures_pnl = (position['futures_entry'] / futures.iloc[-1] - 1) * position['size']
            trades.append({
...
class PairArbitrage:
    ...
    def find_cointegrated_pairs(self, price_dict: Dict[str, pd.Series],
        ...
        for i, sym1 in enumerate(symbols):
            for sym2 in symbols[i+1:]:
                ...
                combined = pd.concat([price_dict[sym1], price_dict[sym2]], axis=1).dropna()
                if len(combined) < 500:
                    continue
        ...
        for i in range(100, len(common_idx)):
            ...
            if position:
                ...
                if should_exit:
                    ...
                    trades.append({'pnl': total_pnl, 'zscore_entry': position['z_entry'], 'zscore_exit': z})
                    capital += total_pnl
                    position = None
...
def main():
    ...
    for sym1, sym2, corr in pairs[:15]:  # Top 15 pairs
        ...
        if result:
            ...
            logger.info(f"  {result['pair']:<20} Corr={corr:.2f} PF={result['pf']:.2f} "
                       f"Ret={result['ret']:+.1f}%")
    ...
    if all_results:
        ...
        for _, r in top.iterrows():
            ...
            logger.info(f"  {name:<20} {r['strategy']:<12} PF={r['pf']:.2f} Ret={r['ret']:+.1f}%")
