    ...
    def calculate_scores(self, date, lookback=20):
        ...
        for ticker, df in self.all_data.items():
            ...
            if past['cur_prc'].iloc[0] > 0:
                price_mom = (current['cur_prc'] - past['cur_prc'].iloc[0]) / past['cur_prc'].iloc[0]
            else:
                price_mom = 0

            # 외국인 비중 변화
            wght_change = current['wght'] - past['wght'].iloc[0] if pd.notna(current['wght']) and pd.notna(past['wght'].iloc[0]) else 0
...
def backtest_portfolio_strategy(all_data: dict, strategy_name: str,
                                top_n: int = 5, rebalance_freq: int = 20,
        ...
        if i - last_rebalance >= rebalance_freq:
            ...
            for ticker, df in all_data.items():
                ...
                if score_type == 'foreign_sum':
                    ...
                    if past['cur_prc'].iloc[0] > 0:
                        score = (current['cur_prc'] - past['cur_prc'].iloc[0]) / past['cur_prc'].iloc[0]
                    else:
                        score = 0
                elif score_type == 'combined':
                    foreign_score = past['chg_qty'].sum()
                    if past['cur_prc'].iloc[0] > 0:
                        mom_score = (current['cur_prc'] - past['cur_prc'].iloc[0]) / past['cur_prc'].iloc[0]
                    else:
                        mom_score = 0
            ...
            # 상위 N개 종목 선정
            sorted_tickers = sorted(scores.keys(), key=lambda x: scores[x]['score'], reverse=True)
    ...
    if pv_df['daily_return'].std() > 0:
        sharpe = pv_df['daily_return'].mean() / pv_df['daily_return'].std() * np.sqrt(252)
    else:
        sharpe = 0
...
def backtest_momentum_rotation(all_data: dict, lookback: int = 20,
                               top_n: int = 3, rebalance_freq: int = 5):
    """모멘텀 로테이션 전략"""
...
def backtest_foreign_rotation(all_data: dict, lookback: int = 20,
                              top_n: int = 3, rebalance_freq: int = 10):
    """외국인 순매수 로테이션 전략"""
...
def backtest_combined_rotation(all_data: dict, top_n: int = 3,
...
def backtest_long_short(all_data: dict, top_n: int = 3, rebalance_freq: int = 10):
    ...
    for i, date in enumerate(dates):
        ...
        if i - last_rebalance >= rebalance_freq:
            ...
            short_selected = sorted_tickers[-top_n:] if len(sorted_tickers) >= 2*top_n else []
            ...
            for ticker, (shares, entry_price) in list(short_holdings.items()):
                if ticker in scores:
                    capital += (entry_price - scores[ticker]['price']) * shares - scores[ticker]['price'] * fee_rate
...
def run_all_portfolio_backtests():
    ...
    for ticker, df in all_data.items():
        ...
        print(f"    {ticker}: {name} ({len(df)} rows, {df['dt'].min().date()} ~ {df['dt'].max().date()})")
    ...
    for top_n in [2, 3, 5]:
        for rebalance in [5, 10, 20]:
            result = backtest_foreign_rotation(all_data, top_n=top_n, rebalance_freq=rebalance)
            if result:
                results.append(result)
                print(f"    {result['strategy']}: 수익률={result['total_return_pct']:.1f}%, "
    ...
    for top_n in [2, 3, 5]:
        for rebalance in [5, 10, 20]:
            result = backtest_momentum_rotation(all_data, top_n=top_n, rebalance_freq=rebalance)
            if result:
                results.append(result)
                print(f"    {result['strategy']}: 수익률={result['total_return_pct']:.1f}%, "
                      f"CAGR={result['cagr_pct']:.1f}%, MDD={result['max_drawdown_pct']:.1f}%, "
    ...
    for top_n in [2, 3, 5]:
        for rebalance in [5, 10, 20]:
            result = backtest_combined_rotation(all_data, top_n=top_n, rebalance_freq=rebalance)
            if result:
                results.append(result)
                print(f"    {result['strategy']}: 수익률={result['total_return_pct']:.1f}%, "
                      f"CAGR={result['cagr_pct']:.1f}%, MDD={result['max_drawdown_pct']:.1f}%, "
    ...
    for top_n in [2, 3]:
        for rebalance in [5, 10, 20]:
            result = backtest_long_short(all_data, top_n=top_n, rebalance_freq=rebalance)
            if result:
                results.append(result)
                print(f"    {result['strategy']}: 수익률={result['total_return_pct']:.1f}%, "
                      f"CAGR={result['cagr_pct']:.1f}%, MDD={result['max_drawdown_pct']:.1f}%, "
    ...
    if results:
        ...
        print(f"수익 전략: {len(profitable)}개 ({len(profitable)/len(results)*100:.1f}%)")

        print("\n[상위 15개 전략]")
        print("-" * 70)
        for i, row in results_df.head(15).iterrows():
            print(f"{row['strategy'][:40]:<40} | "
        ...
        # 결과 저장
        output_file = "E:/투자/Multi-Asset Strategy Platform/portfolio_backtest_results.csv"
