...
print(
    f"Out-of-Sample 기간: {out_sample.index[0].date()} ~ {out_sample.index[-1].date()} ({len(out_sample)}일)"
)
...
strategies = {
    ...
    "CombBest_14_35_75_70_20": lambda df: generate_triple_signals(
        df, 14, -75, -35, 20
    ),  # 간소화
}
...
for r in results:
    ...
    print(
        f"{r['name']:<30} {r['is_sharpe']:>10.3f} {r['os_sharpe']:>10.3f} {r['sharpe_decay']:>9.1f}% {verdict:>10}"
    )
...
for wr_low in [-70, -73, -75, -78, -80, -83, -85]:
    ...
    print(
        f"{wr_low}/{wr_high:<10} {result['sharpe']:>10.3f} {result['trades']:>10} {result['win_rate']:>9.1%}"
    )

# 파라미터 민감도 계산
sharpes = [p["sharpe"] for p in param_results]
sensitivity = (
    np.std(sharpes) / np.mean(sharpes) if np.mean(sharpes) > 0 else float("inf")
)
...
for name in list(strategies.keys())[:4]:
    ...
    if len(sharpes) > 1:
        ...
        print(
            f"  {short_name:<20}: 평균={mean:>6.2f}, 표준편차={std:>5.2f} [{stability}]"
        )
...
for name, signal_func in strategies.items():
    ...
    print(
        f"{name:<30} {num_params:>5} {result['trades']:>10} {trades_per_param:>15.1f} {verdict:>10}"
    )
...
output_path = Path(
    "E:/투자/data/kosdaq_futures/validated_strategies/overfitting_analysis.json"
)
