...
from kosdaq_live_trading_validation import (
    TripleV5Strategy, TripleVolStrategy, TripleADXStrategy,
    CombBestStrategy, LiveTradingConfig, LiveTradingValidator
)
...
print(f"In-Sample: {in_sample.index[0].date()} ~ {in_sample.index[-1].date()} ({len(in_sample)}일)")
print(f"Out-of-Sample: {out_sample.index[0].date()} ~ {out_sample.index[-1].date()} ({len(out_sample)}일)")
...
def simple_backtest(strategy, df, commission=0.0001, slippage=0.001):
    ...
    if len(signals) < 10:
        return {'sharpe': 0, 'return': 0, 'trades': len(signals), 'win_rate': 0, 'mdd': 0}
...
for strategy in strategies:
    ...
    if abs(is_result['sharpe']) > 0.01:
        decay = (os_result['sharpe'] - is_result['sharpe']) / abs(is_result['sharpe']) * 100
    else:
        decay = 0
    ...
    print(f"{strategy.name:<35} {is_result['sharpe']:>10.3f} {os_result['sharpe']:>10.3f} {decay:>9.1f}% {verdict:>12}")
...
for r in results:
    print(f"{r['name']:<35} {r['is_return']*100:>11.1f}% {r['os_return']*100:>11.1f}% {r['is_mdd']*100:>9.1f}% {r['os_mdd']*100:>9.1f}%")

print(f"\n{'전략':<35} {'IS 승률':>10} {'OS 승률':>10} {'IS 거래':>10} {'OS 거래':>10}")
print("-" * 80)

for r in results:
    print(f"{r['name']:<35} {r['is_winrate']*100:>9.1f}% {r['os_winrate']*100:>9.1f}% {r['is_trades']:>10} {r['os_trades']:>10}")
...
for train_period, test_year in periods:
    ...
    train_data = data[(data.index.year >= int(train_start)) & (data.index.year <= int(train_end))]
...
for r in results:
    ...
    print(f"{r['name']:<35} {full_sharpe:>12.3f} {r['os_sharpe']:>12.3f} {wf_mean:>10.3f} {verdict:>15}")
...
# 실거래 추천 전략
viable = [d for d in final_diagnosis if d['os_sharpe'] > 0.5 and d['wf_mean'] > 0.3 and d['score'] < 3]
print("실거래 권장 전략 (OS Sharpe > 0.5, WF 평균 > 0.3):")
if viable:
    for v in sorted(viable, key=lambda x: x['os_sharpe'], reverse=True):
        print(f"  - {v['name']}: OS Sharpe={v['os_sharpe']:.3f}, WF평균={v['wf_mean']:.3f}")
else:
...
output_path = Path("E:/투자/data/kosdaq_futures/validated_strategies/overfitting_analysis_v2.json")
