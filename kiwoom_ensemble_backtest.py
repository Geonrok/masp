...
class VotingEnsemble(EnsembleStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['total_votes'] = df[['vote_1', 'vote_2', 'vote_3', 'vote_4', 'vote_5']].sum(axis=1)
        ...
        for i in range(100, len(df)):
            ...
            if pd.notna(vol):
                if vol < vol_threshold:
                    ...
                    # 고변동성 환경 - 추세 강화
                    if foreign_mom > 0 and df['trend_mid'].iloc[i] == 1 and df['trend_long'].iloc[i] == 1:
                        df.iloc[i, df.columns.get_loc('signal')] = 1
...
class MomentumQuality(EnsembleStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 종합 스코어
        df['total_score'] = (df['mom_score'] + df['quality_score'] + df['flow_score']) / 3
...
class TrendBreakout(EnsembleStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['foreign_confirm'] = (df['chg_qty'].rolling(foreign_confirm).sum() > 0).astype(int)
...
class VolatilityBreakout(EnsembleStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['foreign_positive'] = (df['chg_qty'].rolling(foreign_period).sum() > 0).astype(int)

        # 변동성 돌파 + 외국인 순매수
        df.loc[(df['cur_prc'] > df['target']) & (df['foreign_positive'] == 1), 'signal'] = 1

        return df


def backtest_strategy(df: pd.DataFrame, strategy: EnsembleStrategy,
                     ticker: str, initial_capital: float = 100_000_000,
    ...
    for i in range(1, len(df)):
        ...
        position_size = current.get('position_size', 1.0) if 'position_size' in df.columns else 1.0
...
def run_ensemble_backtests():
    ...
    strategies = [
        # 다중 팩터
        MultiFactorEnsemble("MultiFactor_443_50", {'foreign_weight': 0.4, 'momentum_weight': 0.4, 'trend_weight': 0.3, 'threshold': 0.5}),
        MultiFactorEnsemble("MultiFactor_532_60", {'foreign_weight': 0.5, 'momentum_weight': 0.3, 'trend_weight': 0.2, 'threshold': 0.6}),
        MultiFactorEnsemble("MultiFactor_334_40", {'foreign_weight': 0.3, 'momentum_weight': 0.3, 'trend_weight': 0.4, 'threshold': 0.4}),
        ...
        # 모멘텀 + 퀄리티
        MomentumQuality("MomQuality_20_10", {'mom_lookback': 20, 'foreign_lookback': 10}),
    ...
    if results:
        ...
        for _, row in results_df.head(20).iterrows():
            print(f"{row['name'][:10]:<10} | {row['strategy']:<25} | "
        ...
        for strategy, row in strategy_avg.iterrows():
            print(f"{strategy:<25} | 평균수익: {row['total_return_pct']:>8.1f}% | "
                  f"MDD: {row['max_drawdown']:>6.1f}% | "
                  f"승률: {row['win_rate']:>5.1f}% | "
                  f"거래: {row['num_trades']:>5.1f}회")

        # 결과 저장
        output_file = "E:/투자/Multi-Asset Strategy Platform/ensemble_backtest_results.csv"
