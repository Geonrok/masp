...
async def main():
    ...
    logger.info(f"모의투자: {api.is_simulation if hasattr(api, 'is_simulation') else 'unknown'}")
        ...
        for key in body:
            if 'OutBlock' in key:
                ...
                if isinstance(data, dict):
                    # 주요 필드만 출력
                    important = {k: v for k, v in data.items()
                                 if k in ['hname', 'price', 'sign', 'change', 'diff', 'volume',
                                          'jnilclose', 'offerho1', 'bidho1', 'high', 'low', 'open',
                                          'shcode', 'exhratio']}
    ...
    # 4. t1104 - 종목현재가 다중조회
    logger.info("\n=== t1104 멀티 현재가 ===")
    response = await api.request("t1104", {
        "code1": "247540",  # 에코프로비엠
        "code2": "196170",  # 알테오젠
        "code3": "263750",  # 펄어비스
    })
    ...
    logger.info("\n=== t1514 업종기간별추이 (코스닥) ===")
    response = await api.request("t1514", {
        "upcode": "001",  # 코스닥 지수
        "gubun2": "2",  # 2:일별
        "cts_date": "",
        "cnt": "20",
    ...
    logger.info("\n=== t1516 업종별종목시세 ===")
    response = await api.request("t1516", {
        "upcode": "001",  # 코스닥
        "gubun": "1",  # 1:시가총액
        "shcode": "",
    })
