...
class MACDForeign(AdvancedStrategy):
    ...
    def generate_signals(self, df):
        ...
        df.loc[(df["macd"] < df["macd_signal"]) & (df["foreign_sum"] < 0), "signal"] = (
            -1
        )
...
class VolumeBreakoutForeign(AdvancedStrategy):
    ...
    def generate_signals(self, df):
        ...
        df.loc[
            (df["trde_qty"] > df["vol_ma"] * vol_mult) & (df["foreign_sum"] > 0),
            "signal",
        ] = 1
        # 거래량 급증 + 외국인 순매도 = 매도
        df.loc[
...
class DualMAForeign(AdvancedStrategy):
    ...
    def generate_signals(self, df):
        ...
        df.loc[(df["ma_short"] < df["ma_long"]) & (df["foreign_sum"] < 0), "signal"] = (
            -1
        )
...
def run_all_backtests():
    ...
    strategies = [
        # 추세 필터 + 수급
        TrendFilterForeign(
            "TrendFilter_MA50_F5", {"ma_period": 50, "foreign_period": 5}
        ),
        TrendFilterForeign(
            "TrendFilter_MA20_F5", {"ma_period": 20, "foreign_period": 5}
        ),
        TrendFilterForeign(
            "TrendFilter_MA100_F10", {"ma_period": 100, "foreign_period": 10}
        ),
        # 돌파 + 수급
        ...
        RSIForeign(
            "RSI_30_70_F5",
            {"rsi_period": 14, "oversold": 30, "overbought": 70, "foreign_period": 5},
        ),
        RSIForeign(
            "RSI_25_75_F5",
            {"rsi_period": 14, "oversold": 25, "overbought": 75, "foreign_period": 5},
        ),
        # MACD + 수급
        MACDForeign(
            "MACD_12_26_9_F5",
            {"fast": 12, "slow": 26, "signal": 9, "foreign_period": 5},
        ),
        MACDForeign(
            "MACD_12_26_9_F10",
            {"fast": 12, "slow": 26, "signal": 9, "foreign_period": 10},
        ),
        # 볼린저 + 수급
        BollingerForeign(
            "Bollinger_20_2_F5", {"period": 20, "std_mult": 2, "foreign_period": 5}
        ),
        BollingerForeign(
            "Bollinger_20_2.5_F5", {"period": 20, "std_mult": 2.5, "foreign_period": 5}
        ),
        # 거래량 돌파 + 수급
        VolumeBreakoutForeign(
            "VolBreakout_2x_F5", {"vol_period": 20, "vol_mult": 2, "foreign_period": 5}
        ),
        VolumeBreakoutForeign(
            "VolBreakout_3x_F5", {"vol_period": 20, "vol_mult": 3, "foreign_period": 5}
        ),
        # 모멘텀 + 수급
        ...
        # 이중 MA + 수급
        DualMAForeign(
            "DualMA_10_50_F5", {"short_ma": 10, "long_ma": 50, "foreign_period": 5}
        ),
        DualMAForeign(
            "DualMA_20_100_F10", {"short_ma": 20, "long_ma": 100, "foreign_period": 10}
        ),
        # 외국인 지분율 가속화
    ...
    for file in sorted(merged_files):
        ...
        if best_result and best_return > 0:
            print(
                f"  최고: {best_result['strategy']} = {best_result['total_return']*100:.1f}% "
    ...
    if all_results:
        ...
        results_df.to_csv(
            f"{RESULT_PATH}/advanced_strategy_results.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        strategy_avg = (
            ...
            .agg({"total_return": "mean", "annual_return": "mean", "win_rate": "mean"})
            .sort_values("total_return", ascending=False)
        )

        for i, (strategy, row) in enumerate(strategy_avg.head(10).iterrows()):
            print(
                f"  {i+1}. {strategy}: 평균수익 {row['total_return']*100:.1f}%, "
