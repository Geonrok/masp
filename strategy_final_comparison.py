...
def backtest_strategy(strategy, df, commission=0.0001, slippage=0.001):
    ...
    if len(signals) < 5:
        return {
            'sharpe': 0, 'return': 0, 'trades': len(signals),
            'win_rate': 0, 'mdd': 0, 'cagr': 0,
...
for i, r in enumerate(full_results, 1):
    print(f"{i:<4} {r['name']:<35} {r['sharpe']:>8.3f} {r['cagr']*100:>7.1f}% {r['mdd']*100:>7.1f}% {r['win_rate']*100:>7.1f}%")
...
# 구간별 성과
market_results = {}
for market_name, market_data in [('상승장', bull_data), ('하락장', bear_data), ('횡보장', sideways_data)]:
...
for market_name in ['상승장', '하락장', '횡보장']:
    ...
    results = sorted(market_results[market_name], key=lambda x: x['sharpe'], reverse=True)
    for r in results:
        print(f"{r['name']:<35} {r['sharpe']:>8.3f} {r['return']*100:>9.1f}% {r['win_rate']*100:>7.1f}% {r['trades']:>8}")
...
print(f"\n### 시장 구간별 성과 ###")
print(f"{'구간':<12} {'Sharpe':>10} {'수익률':>12} {'승률':>10} {'거래수':>10} {'평균손익':>12}")
print("-" * 70)

for market_name, market_data in [('상승장', bull_data), ('하락장', bear_data), ('횡보장', sideways_data), ('전체', data)]:
    if len(market_data) < 50:
        continue
    result = backtest_strategy(best_strategy, market_data)
    print(f"{market_name:<12} {result['sharpe']:>10.3f} {result['return']*100:>11.1f}% {result['win_rate']*100:>9.1f}% {result['trades']:>10} {result['avg_trade']:>11.3f}%")
...
year_market = {
    2010: '횡보', 2011: '하락', 2012: '상승', 2013: '상승',
    2014: '상승', 2015: '급락', 2016: '상승', 2017: '상승',
    2018: '하락', 2019: '회복', 2020: '급등락', 2021: '하락',
    2022: '하락', 2023: '회복', 2024: '횡보', 2025: '횡보'
}
...
for year in range(2010, 2027):
    ...
    print(f"{year:<6} {market:<12} {result['sharpe']:>10.3f} {result['return']*100:>11.1f}% {result['win_rate']*100:>9.1f}% {result['trades']:>8}")
...
if rank == 1:
    ...
    print(f"       TripleV5_14_38은 {rank}위이지만 Out-of-Sample 성과 기준으로는 권장됩니다.")
