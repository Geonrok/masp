...
def get_target_tickers():
    ...
    top_kosdaq = kosdaq.sort_values("시가총액", ascending=False).head(200).index.tolist()
...
def download_one(ticker):
    ...
    try:
        ...
        rename_map = {
            '연기금등': 'Pension', '연기금': 'Pension',
            '외국인': 'Foreigner', '기관합계': 'Institution',
            '개인': 'Individual', '사모': 'PrivateEquity',
            '금융투자': 'FinancialInv', '보험': 'Insurance',
            '투신': 'Trust', '은행': 'Bank', '기타금융': 'EtcFinance',
            '기타법인': 'EtcCorp', '전체': 'Total'
        }
