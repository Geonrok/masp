...
class ForeignExtreme(ForeignInvestorStrategy):
    ...
    def generate_signals(self, df):
        ...
        df["foreign_upper"] = (
            df["chg_qty"]
            .rolling(lookback)
            .apply(lambda x: np.percentile(x, 100 - percentile), raw=True)
        )
        df["foreign_lower"] = (
            df["chg_qty"]
            .rolling(lookback)
...
class ForeignMeanReversion(ForeignInvestorStrategy):
    ...
    def generate_signals(self, df):
        ...
        df.loc[
            df["chg_qty"] < df["foreign_ma"] - std_mult * df["foreign_std"], "signal"
        ] = 1
...
def run_all_backtests():
    ...
    strategies = [
        ...
        # 외국인 지분율 임계값
        ForeignRatioThreshold(
            "Foreign_Threshold_30_20", {"buy_threshold": 30, "sell_threshold": 20}
        ),
        ForeignRatioThreshold(
            "Foreign_Threshold_40_30", {"buy_threshold": 40, "sell_threshold": 30}
        ),
        ForeignRatioThreshold(
            "Foreign_Threshold_50_40", {"buy_threshold": 50, "sell_threshold": 40}
        ),
        # 외국인 지분율 이동평균
        ...
        for strategy in strategies:
            ...
            if result:
                ...
                if result["total_return"] > 0:
                    print(
                        f"  {strategy.name}: {result['total_return']*100:.1f}% "
    ...
    if all_results:
        ...
        # CSV 저장
        results_df.to_csv(
            f"{RESULT_PATH}/foreign_strategy_results.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        for ticker in results_df["ticker"].unique():
            ...
            name = best["name"]
            print(
                f"  {name} ({ticker}): {best['strategy']} = {best['total_return']*100:.1f}%"
            )
