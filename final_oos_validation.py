...
def backtest_strategy(df: pd.DataFrame, strategy_func,
    ...
    for i in range(1, len(df)):
        ...
        if position == 0 and signal == 1:
            ...
            if shares > 0:
                ...
                capital -= cost
                position = 1
                trades.append({'type': 'buy', 'price': buy_price, 'date': current['dt']})
    ...
    win_count = sum(1 for i in range(min(len(buy_trades), len(sell_trades)))
...
def run_oos_validation():
    ...
    for name, ticker, strategy_name, strategy_func in recommended_strategies:
        ...
        # In-Sample (2018 ~ 2023)
        is_df = df[(df['dt'] >= '2018-01-01') & (df['dt'] < oos_start)].copy().reset_index(drop=True)
        # Out-of-Sample (2024 ~)
    ...
    for r in results:
        ...
        consistent = r['oos_return'] > r['is_return'] * 0.2 if r['is_return'] > 0 else r['oos_return'] > 0
        ...
        print(f"{r['name']:<12} {r['strategy']:<22} │ "
    ...
    if passing:
        ...
        for r in sorted(passing, key=lambda x: x['oos_return'], reverse=True):
            ...
            print(f"    - 수익률: {r['is_return']:.1f}%, MDD: {r['is_mdd']:.1f}%, 거래: {r['is_trades']}회, 승률: {r['is_winrate']:.1f}%")
            print(f"  Out-Sample ({r['oos_period']})")
            print(f"    - 수익률: {r['oos_return']:.1f}%, MDD: {r['oos_mdd']:.1f}%, 거래: {r['oos_trades']}회, 승률: {r['oos_winrate']:.1f}%")

        # 결과 저장
        results_df = pd.DataFrame(passing)
        results_df.to_csv('E:/투자/Multi-Asset Strategy Platform/final_recommended_strategies.csv',
