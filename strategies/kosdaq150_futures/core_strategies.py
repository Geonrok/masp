...
class TripleV5Strategy:
    ...
    def generate_signals(self, df: pd.DataFrame) -> List[TradingSignal]:
        ...
        for i in range(start, len(df)):
            ...
            bull_count = 0
            if (
                cmo.iloc[i - 2] < -self.cmo_threshold
                and cmo.iloc[i - 1] >= -self.cmo_threshold
            ):
                bull_count += 1
            if (
                wr.iloc[i - 2] < -self.wr_threshold
                and wr.iloc[i - 1] >= -self.wr_threshold
            ):
                bull_count += 1
            ...
            if (
                ...
                bear_count += 1
            if wr.iloc[i - 2] > -(100 - self.wr_threshold) and wr.iloc[i - 1] <= -(
                100 - self.wr_threshold
            ):
                bear_count += 1
