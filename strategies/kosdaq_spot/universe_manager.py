...
class KOSDAQUniverseManager:
    ...
    def load_stock_info(self) -> Dict:
        ...
        self._stock_info = {}
        foreign_files = [f for f in os.listdir(self.config.foreign_path)
        ...
        for f in foreign_files:
            ...
            try:
                ...
                # 병합
                merged = pd.merge(ohlcv_df, foreign_df[['date', 'chg_qty', 'wght', 'poss_stkcnt']],
                                 on='date', how='inner')
                ...
                if len(merged) >= self.config.min_data_days:
                    # 최근 1년 평균 거래대금 (유동성)
                    recent = merged[merged['date'] >= merged['date'].max() - pd.Timedelta(days=365)]
    ...
    def get_universe(self, force_refresh: bool = False) -> List[str]:
        ...
        # 유동성 기준 상위 50%
        sorted_by_liquidity = sorted(stock_info.items(),
        ...
        liquid_info = {t: stock_info[t] for t in liquid_tickers
                      if t in stock_info and stock_info[t]['marketcap'] > 0}
        sorted_by_mcap = sorted(liquid_info.items(),
    ...
    def save_universe(self, output_path: Optional[str] = None) -> str:
        ...
        data = {
            ...
            'universe': [
                ...
                for t in universe if t in stock_info
            ]
        }
