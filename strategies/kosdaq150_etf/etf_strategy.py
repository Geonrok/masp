...
class KOSDAQ150ETFStrategy:
    ...
    @staticmethod
    def _bollinger_pctb(close: pd.Series, period: int = 20,
    ...
    def _generate_triple_v5_signals(self, df: pd.DataFrame,
                                     cmo_t: int, wr_t: int,
                                     strategy_name: str) -> List[Signal]:
        """TripleV5 전략 신호 생성"""
        signals = []
        ...
        for i in range(25, len(df)):
            ...
            if bull_count >= 2:
                signals.append(Signal(date, 1, bull_count/3, strategy_name,
                                      f"상향돌파 {bull_count}/3"))
            elif bear_count >= 2:
                signals.append(Signal(date, -1, bear_count/3, strategy_name,
                                      f"하향돌파 {bear_count}/3"))
    ...
    def _generate_triple_vol_signals(self, df: pd.DataFrame) -> List[Signal]:
        ...
        for i in range(35, len(df)):
            ...
            if bull_count >= 2:
                signals.append(Signal(date, 1, bull_count/3, "TripleVol_38",
                                      f"상향돌파+거래량 {bull_count}/3"))
            elif bear_count >= 2:
                signals.append(Signal(date, -1, bear_count/3, "TripleVol_38",
    ...
    def backtest(self, df: pd.DataFrame) -> Dict:
        ...
        win_rate = (combined > 0).sum() / (combined != 0).sum() if (combined != 0).sum() > 0 else 0
