...
def load_merged_data():
    """키움 merged 데이터 로드"""
    merged_files = [f for f in os.listdir(KIWOOM_DATA_PATH) if f.endswith('_merged.csv')]
    all_data = {}
...
def calculate_realistic_metrics(df: pd.DataFrame, signals: pd.Series,
                                initial_capital: float = 100_000_000,
                                fee_rate: float = 0.0015,  # 0.15% (세금+수수료)
                                slippage: float = 0.001,   # 0.1% 슬리피지
                                max_position_pct: float = 0.1):  # 거래량의 10% 이하
    """
    현실적인 백테스트 메트릭 계산
    ...
    avg_loss = total_loss / (len(buy_trades) - win_count) if (len(buy_trades) - win_count) > 0 else 1
    ...
    return {
        ...
        'avg_volume_pct': np.mean([t.get('volume_pct', 0) for t in buy_trades]) if buy_trades else 0
    }
...
def strategy_trend_filter_foreign(df: pd.DataFrame, ma_period: int = 100, foreign_period: int = 10):
    """추세 필터 + 외국인"""
    df = df.copy()
...
def strategy_momentum_foreign(df: pd.DataFrame, mom_period: int = 20, foreign_period: int = 10):
    """모멘텀 + 외국인"""
    df = df.copy()
...
def strategy_ma_crossover_foreign(df: pd.DataFrame, short: int = 20, long: int = 60, foreign_period: int = 5):
    """이평선 크로스 + 외국인"""
    df = df.copy()
...
def strategy_breakout_foreign(df: pd.DataFrame, breakout_period: int = 20, foreign_period: int = 5):
    """돌파 + 외국인"""
    df = df.copy()
...
def strategy_volatility_breakout(df: pd.DataFrame, k: float = 0.5, foreign_period: int = 3):
    """변동성 돌파 + 외국인"""
    df = df.copy()
...
def strategy_dual_momentum_foreign(df: pd.DataFrame, short: int = 10, long: int = 30, foreign_period: int = 5):
    """듀얼 모멘텀 + 외국인"""
    df = df.copy()
...
def strategy_rsi_foreign(df: pd.DataFrame, rsi_period: int = 14, oversold: int = 30, foreign_period: int = 5):
    """RSI + 외국인"""
    df = df.copy()
...
def run_validation():
    ...
    # 전략 정의
    strategies = [
        ('TrendFilter_MA100_F10', lambda df: strategy_trend_filter_foreign(df, 100, 10)),
        ...
        ('DualMom_20_60_F10', lambda df: strategy_dual_momentum_foreign(df, 20, 60, 10)),
        ...
        for strategy_name, strategy_func in strategies:
            try:
                ...
                if wf:
                    ...
                    consistency = len([r for r in wf if r['total_return_pct'] > 0]) / len(wf)
    ...
    if all_results:
        ...
        print(f"  실거래 가능 기준 통과: {len(practical)}개 ({len(practical)/len(results_df)*100:.1f}%)")
        ...
        if len(practical) > 0:
            ...
            print("-" * 100)
            print(f"{'종목':<12} {'전략':<25} {'수익률':>10} {'CAGR':>8} {'MDD':>8} {'승률':>6} {'샤프':>6} {'거래':>5}")
            print("-" * 100)

            for _, row in practical.head(20).iterrows():
                print(f"{row['name'][:10]:<12} {row['strategy']:<25} "
    ...
    if wf_results:
        ...
        print(f"  일관성 기준 통과: {len(consistent)}개 ({len(consistent)/len(wf_df)*100:.1f}%)")
        ...
        if len(consistent) > 0:
            ...
            print("\n[일관성 높은 전략 - 상위 20개]")
            print("-" * 90)
            print(f"{'종목':<12} {'전략':<25} {'평균수익':>10} {'평균샤프':>8} {'승률':>6} {'일관성':>8}")
            print("-" * 90)
    ...
    if len(practical) > 0 and len(consistent) > 0:
        ...
        if both_pass:
            ...
            for ticker, strategy in both_pass:
                p_row = practical[(practical['ticker'] == ticker) & (practical['strategy'] == strategy)].iloc[0]
                c_row = consistent[(consistent['ticker'] == ticker) & (consistent['strategy'] == strategy)].iloc[0]
    ...
    if all_results:
        ...
        for strategy, row in strategy_stats.iterrows():
            print(f"{strategy:<25} | 수익: {row['total_return_pct']:>7.1f}% | "
                  f"샤프: {row['sharpe_ratio']:>5.2f} | MDD: {row['max_drawdown_pct']:>6.1f}%")

        strategy_stats.to_csv(os.path.join(RESULTS_PATH, 'strategy_stats.csv'),
                             encoding='utf-8-sig')
